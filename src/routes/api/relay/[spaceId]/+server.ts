...
import {
  ...
  relayEnsureHydrated,
  logEpoch
} from '$lib/server/relay/log';
import { notifySpaceChanged } from '$lib/server/relay/notify';
...
export const POST: RequestHandler = async ({ request, params, cookies }) => {
  ...
  // T3: first touch after a process start reloads sealed history from the
  // mirror, so dedupe sees it and seq numbering includes it.
  await relayEnsureHydrated(spaceId);
  ...
  // T1: wake subscribed replicas — metadata only (spaceId, no content), and
  // only when something genuinely new landed (dedupes don't re-ring).
  if (results.some((r) => r.ok && !r.deduped)) {
...
export const GET: RequestHandler = async ({ url, params, cookies }) => {
  ...
  // T3: lazy re-hydration of sealed history from the mirror after a restart.
  await relayEnsureHydrated(spaceId);
