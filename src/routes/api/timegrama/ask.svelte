  ...
  ask (id:${id}) {data{ id attributes{
    ...
    try {
      ...
      if (res.data != null) {
        ...
        if (res.data.ask.data.attributes.archived != true) {
          ...
          if (
            res.data.ask.data.attributes.vots.length > 0 ||
            (res.data.ask.data.attributes.project.data.attributes.user_1s.data
              .map((c) => c.id)
              .includes(
                res.data.ask.data.attributes.users_permissions_user.data.id
              ) &&
              res.data.ask.data.attributes.vots
                .map((c) => c.users_permissions_user.data.id)
                .includes(
                  res.data.ask.data.attributes.users_permissions_user.data.id
                ))
          ) {
    ...
    open_mission{data{id attributes{hearotMeyuchadot
       tafkidims{data{id}} name privatlinks sqadualed dates howMeny publicklinks descrip noofhours perhour iskvua mission{data{id}}}}} 
            ...
            try {
              ...
              if (res2.data != null) {
                console.log(
                  id,
                  res2.data.ask.data.attributes.open_mission.data.attributes
                    .sqadualed
                );
                console.log([
                  ...res.data.ask.data.attributes.project.data.attributes.user_1s.data.map(
                    (c) => c.id
                  ),
                  res.data.ask.data.attributes.users_permissions_user.data.id
                ]);
                ...
                if (
                  !res.data.ask.data.attributes.project.data.attributes.user_1s.data
                    .map((c) => c.id)
                    .includes(
                      res.data.ask.data.attributes.users_permissions_user.data
                        .id
                    )
                ) {
                  //TODO: test it!
      ...
      data: {project: "${res.data.ask.data.attributes.project.data.id}",
             ...
             publishedAt: "${d.toISOString()}",
             open_missions:[${res2.data.ask.data.attributes.open_mission.data.id}],
          ...
          `;
                    ...
                    if (
                      !res.data.ask.data.attributes.project.data.attributes.user_1s.data
                        .map((c) => c.id)
                        .includes(
                          res.data.ask.data.attributes.users_permissions_user
                            .data.id
                        )
                    ) {
                      let emailt;
                      let ema =
                        res3.data.updateProject.data.attributes.user_1s.data;
                      let la;
                      ...
                      console.log(langi);
                      let data = {
                        user: res.data.ask.data.attributes
                          .users_permissions_user.data.attributes.username,
                        projectName:
                          res3.data.createMesimabetahalich.data.attributes
                            .project.data.attributes.projectName,
                        projectSrc:
                          res3.data.createMesimabetahalich.data.attributes
                            .project.data.attributes.profilePic.data.attributes
                            .url,
                        missionName:
                          res2.data.ask.data.attributes.open_mission.data
                            .attributes.name,
                        email: emailt,
                        lang: langi,
                        kind: 'exeptedMission'
                    ...
                    const otherasks =
                      res3.data.updateOpenMission.data.attributes.asks.data;
