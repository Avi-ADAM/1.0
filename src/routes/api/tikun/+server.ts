/**
 * Anonymous pre-signup proxy for the "tikunolam" (suggest-text-change) form.
 *
 * tikunolam.svelte is rendered inside amana.svelte before login/signup, so
 ...
 * site), whose tikunar.svelte/tikuneng.svelte (Arabic/English variants) hit
 * Strapi's /graphql directly too — see corsHeaders below.
 */
...
export const POST: RequestHandler = async ({ request, fetch }) => {
	const { amort, amorts, amortt, amortf, amorth, more, notes, name, email } = await request.json();
