<script>
  ...
  let srcP = project?.attributes?.profilePic?.data 
    ? (project.attributes.profilePic.data.attributes.formats?.thumbnail?.url || project.attributes.profilePic.data.attributes.url)
    : null;
  ...
  const texts = {
    ...
    joinDesc: {"he": "כדי לקחת חלק פעיל, לראות את כל המידע וליצור שיתופי פעולה, מומלץ להירשם.", "en": "To participate, view full info and collaborate, please register."},
  ...
  let pageTitle = $derived({"he": `1💗1 | ${project?.attributes?.projectName || "ריקמה"}` ,"en": `1💗1 | ${project?.attributes?.projectName || "FreeMate"}`});
</script>
...
{#if project}
  ...
  <div bind:clientWidth={w} class="min-h-screen bg-gradient-to-br from-[#1a0515] via-[#2c0b1e] to-[#120f26] text-white overflow-y-auto overflow-x-hidden font-sans">
    ...
    <div dir="rtl" class="max-w-4xl mx-auto px-4 py-8 pb-32 relative">
      ...
      <div class="flex flex-col items-center justify-center mb-8 animate-fade-in-up">
        ...
        <div class="flex gap-4 p-3 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm shadow-lg mb-6">
          {#if discordlink}
            <a target="_blank" href={discordlink} class="social-btn group" title="Discord">
              <img src="https://res.cloudinary.com/love1/image/upload/v1662563246/discord-icon-svgrepo-com_d4vk6m.svg" alt="Discord"/>
            </a>
          {/if}
          {#if linkP}
            <a target="_blank" href={linkP} class="social-btn group text-gold" title={texts.visit[$lang]}>
               <!-- Website Icon -->
	...
	C512,160.275,501.58,170.695,488.727,170.695z M418.909,124.149h46.545V77.576h-46.545V124.149z"/>
          ...
          {#if githublink}
            <a target="_blank" href={githublink} class="social-btn group">
              <img src="https://visualpharm.com/assets/720/Github-595b40b65ba036ed117d442f.svg" alt="GitHub"/>
            </a>
          {/if}
          {#if fblink}
            <a target="_blank" href={fblink} class="social-btn group">
              <img src="https://res.cloudinary.com/love1/image/upload/v1639258134/NicePng_oro-png_2336309_rkhbf8.png" alt="Facebook"/>
            </a>
          {/if}
        </div>
      </div>
      ...
      {#if !isRegisteredUser}
        <div class="mb-10 mx-2 p-6 rounded-2xl bg-gradient-to-r from-pink-900/80 to-purple-900/80 border border-gold/40 shadow-[0_0_15px_rgba(255,0,174,0.3)] text-center relative overflow-hidden">
          ...
          <h2 class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-l from-gold via-white to-gold mb-2">
            {texts.joinTitle[$lang]}
          </h2>
          <p class="text-gray-200 mb-6 max-w-lg mx-auto leading-relaxed">
            {texts.joinDesc[$lang]}
          </p>
          <a href="/" class="inline-block px-8 py-3 rounded-full bg-gradient-to-r from-gold via-[#d4af37] to-[#b8860b] text-black font-bold shadow-lg hover:scale-105 transition-transform hover:shadow-gold/50">
            {texts.login[$lang]}
          </a>
        </div>
      {/if}
        ...
        <div class="mb-10 text-center">
          ...
          <div dir="ltr" class="flex flex-wrap justify-center gap-2">
            {#each projectUsers as user}
              ...
              >
                <div class="w-12 h-12 rounded-full border-2 border-gold p-0.5 bg-black/50 overflow-hidden shadow-md group-hover:shadow-gold/50">
                   <img 
                    class="w-full h-full rounded-full object-cover" 
                    src="{user.attributes.profilePic.data ? user.attributes.profilePic.data.attributes.url : "https://res.cloudinary.com/love1/image/upload/v1653053361/image_s1syn2.png"}" 
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        ...
        <!-- Values -->
        {#if vallues.length > 0}
          <div class="glass-panel flex flex-col items-center border-t-4 border-t-gold">
            <h2 class="text-xl font-bold text-gold mb-4 drop-shadow-md">{texts.values[$lang]}</h2>
        ...
        <!-- Missions -->
        <div class="glass-panel flex flex-col items-center border-t-4 border-t-barbi">
          <h3 class="text-xl font-bold text-barbi mb-4 drop-shadow-md">{texts.missions[$lang]}</h3>
          <div class="flex flex-wrap justify-center gap-2 w-full"> 
            {#if projecto.length > 0}
              {#each projecto as om}
                <button onclick={()=>mesima(om.id)} class="transform hover:scale-105 transition-transform">
                  <Tile bg="wow" sm={true} big={true} word={om.attributes.name}/>
                </button>
              {/each}
            {:else}
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {#each project.attributes.matanotofs.data as matanot}
                  <div class="bg-black/40 rounded-xl p-4 border border-white/10 hover:border-gold/50 transition-colors">
                    <p class="text-lg font-semibold text-white mb-2">{matanot.attributes.name}</p>
                    <p class="text-barbi font-bold text-xl mb-2">{matanot.attributes.price}</p>
                    <a href="gift/{matanot.id}" class="text-sm underline text-gold hover:text-white">לפרטים ורכישה</a>
                  </div>
                {/each}
              </div>
            </div>
...
{:else}
  <!-- Loading State -->
  <div class="h-screen w-screen flex items-center justify-center bg-gradient-to-br from-[#1a0515] via-[#2c0b1e] to-[#120f26]">
...
<style>
  ...
  .bg-barbi { background-color: #ff00ae; }
  .border-gold { border-color: #FFD700; }
  .border-barbi { border-color: #ff00ae; }
