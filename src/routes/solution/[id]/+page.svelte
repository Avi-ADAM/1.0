  ...
  >
    {#key (w, points, h)}
      ...
      >
        ...
        {#each points as point, i}
          ...
          <foreignObject
            ...
            x={point.location <= 50
              ? (w / 100) * point.location + 50
              : (w / 100) * point.location - 100}
            stroke="blueviolet"
            width="200"
            height="100"
          >
            ...
            >
              {#if point.edit != true}
                <button
                  on:click={() => (point.edit = true)}
                  on:keypress={() => (point.edit = true)}
                >
                  <Tile
                    big={point.hover}
                    sm={point.hover}
                    word={point.heading[lang]}
                    bg={point.color ? point.color : 'red'}
                  />
                </button>
              {:else}
                <div class="flex flex-row-reverse">
                  <input
                    type="text"
                    class="textinput text-center focus:border-pink-800 border-b-2 border-zinc-400 shadow-md text-barbi bg-{point.color !=
                    'wow'
                      ? point.color
                      : 'green'}-200"
                    bind:value={point.heading[lang]}
                  />
                  <button on:click={() => (point.edit = false)}>✅</button>
                </div>
              {/if}
            </div>
          </foreignObject>
      ...
      {/if}-->
          ...
          {#if points[i + 1] != null}
            ...
            >
              <button
                ...
                class="bg-gold text-barbi rounded-full p-2">➕</button
              >
            </foreignObject>
          {/if}
        {/each}
        ...
        >
          <button
            on:click={addPoint('buttom', points.length)}
            class="bg-gold text-barbi rounded-full p-2">➕</button
          >
        </foreignObject>
      </svg>
