<script>
    ...
    for (let i = 0; i < start.length; i++) {
      for (let j = 0; j < start[i].attributes.asks.data.length; j++) {
        ...
        dictasked.push({
          ...
          orderon:
            t.open_mission.data.attributes.negopendmissions.data.length || 0,
          negopendmissions:
            t.open_mission.data.attributes.negopendmissions.data,
          name: rt[0],
          stylef: rt[1],
          st: rt[2],
    ...
    if (dictasked.length > 0) {
      ...
      for (let t = 0; t < dictasked.length; t++) {
        ...
        console.log(
          'noofusersNo',
          dictasked[t].users,
          dictasked[t].noofusersOk,
          dictasked[t].noofusersNo,
          dictasked[t].omid,
          'orderon:',
          dictasked[t].orderon
        );
        ...
        if (
          dictasked[t].negopendmissions &&
          dictasked[t].negopendmissions.length > 0
        ) {
          for (let x = 0; x < dictasked[t].negopendmissions.length; x++) {
            let src22 = getProjectData(
              dictasked[t].projectId,
              'upic',
              dictasked[t].negopendmissions[x].attributes.users_permissions_user
                .data.id
            );
            ...
            let negoMessage =
              ...
              negoUser +
              ' ' +
              (tr?.nego?.didNego?.[$lang] || 'negotiated') +
              '</span>';
            ...
            if (
              negoAttrs.noofhours &&
              negoAttrs.noofhours !== dictasked[t].nhours
            ) {
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.thatMission?.[$lang] || 'Mission') +
                ' ' +
                (dictasked[t].nhours || 0) +
                ' ' +
                (tr?.nego?.hoursInsted?.[$lang] || 'hours instead') +
                ' ' +
                (negoAttrs.noofhours || 0) +
                ' ' +
                (tr?.common?.hours?.[$lang] || 'hours');
            }

            if (
              negoAttrs.perhour &&
              negoAttrs.perhour !== dictasked[t].perhour
            ) {
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.perhourNego?.[$lang] || 'Per hour') +
                ' ' +
                (dictasked[t].perhour || 0) +
                ' ' +
                (tr?.nego?.insted?.[$lang] || 'instead') +
                ' ' +
                (negoAttrs.perhour || 0);
            }
            ...
            if (
              ...
              const originalTotal =
                (dictasked[t].perhour || 0) * (dictasked[t].nhours || 0);
              const negoTotal =
                (negoAttrs.perhour || 0) * (negoAttrs.noofhours || 0);
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.total?.[$lang] || 'Total') +
                ' ' +
                originalTotal +
                ' ' +
                (tr?.nego?.insted?.[$lang] || 'instead') +
            ...
            if (negoAttrs.name && negoAttrs.name !== dictasked[t].openName) {
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.nameNego?.[$lang] || 'Name') +
                ' "' +
                dictasked[t].openName +
                '" ' +
                (tr?.nego?.insted?.[$lang] || 'instead') +
                ': "' +
                negoAttrs.name +
            ...
            if (
              negoAttrs.descrip &&
              negoAttrs.descrip !== dictasked[t].missionDetails
            ) {
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.desNego?.[$lang] || 'Description') +
                ' "' +
                dictasked[t].missionDetails +
                '" ' +
                (tr?.nego?.insted?.[$lang] || 'instead') +
                ': "' +
                negoAttrs.descrip +
            ...
            if (
              negoAttrs.hearotMeyuchadot &&
              negoAttrs.hearotMeyuchadot !== dictasked[t].hearotMeyuchadot
            ) {
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.heaNego?.[$lang] || 'Special notes') +
                ' "' +
                dictasked[t].hearotMeyuchadot +
                '" ' +
                (tr?.nego?.insted?.[$lang] || 'instead') +
                ': "' +
                negoAttrs.hearotMeyuchadot +
            ...
            if (negoAttrs.isMonth !== dictasked[t].iskvua) {
              const originalType = dictasked[t].iskvua
                ? tr?.nego?.montly?.[$lang] || 'monthly'
                : tr?.nego?.ont?.[$lang] || 'one-time';
              const negoType = negoAttrs.isMonth
                ? tr?.nego?.montly?.[$lang] || 'monthly'
                : tr?.nego?.ont?.[$lang] || 'one-time';
              negoMessage +=
                '<br>⚙️ ' +
                (tr?.nego?.moaNego?.[$lang] || 'Type') +
                ' "' +
                originalType +
                '" ' +
                (tr?.nego?.insted?.[$lang] || 'instead') +
  ...
  onMount(async () => {
    ...
    if (storedVersion != levVersion) {
      ...
      if (storedSnapshot) {
        ...
        console.log('here arr1 from snapshot', arr1);
      }
  ...
  const unsubscribeNotifications = socketClient.onNotification(
    async (notification) => {
      console.log(
        '🔔 [LEV PAGE] Received notification from socketClient:',
        notification
      );
      ...
      if (updateStrategy?.type === 'fullRefresh') {
        ...
        console.log(
          '🔄 [LEV PAGE] Partial update triggered by:',
          actionKey,
          'Keys:',
          updateStrategy.config?.dataKeys
        );
        ...
        console.log(
          '⚡ [LEV PAGE] Optimistic update - no refresh needed:',
          actionKey
        );
        ...
        console.log(
          '📢 [LEV PAGE] Notification only (no update strategy):',
          actionKey
        );
