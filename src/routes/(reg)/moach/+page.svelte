...
async function start () {
 ...
 const parseJSON = (resp) => (resp.json ? resp.json() : resp);
        ...
        try {
            const res = await fetch("https://beosher.onrender.com/graphql" , {//+ $idPr
              method: "POST",
              headers: {
                'Authorization': bearer1,
                 'Content-Type': 'application/json'
...
let url1 = "https://beosher.onrender.com/api/upload";
let meDatap = [];
let mecata = [];
async function sendP () {
  ...
  idL = cookieValueId;
    token  = cookieValue; 
    let bearer1 = 'bearer' + ' ' + token;
    let linkdi ="https://beosher.onrender.com/api/projects/" + $idPr ;
  //  let fd = new FormData();
