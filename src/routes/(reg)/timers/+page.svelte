<script>
    ...
    import { sendToSer } from '$lib/sendtoser'; // Assuming you have this utility
    ...
    let w = 500; // Initial width, will be updated
    let h = 500; // Initial height, will be updated
    ...
    function formatTime(milliseconds) {
        if (!milliseconds) return "00:00:00"; // Handle null/undefined
      ...
      function checkLine(i) {
          if (i == 0) {
              return { myline: 0, lineCircels: 1, x: center.x - (bigsize / 2), y: center.y - (bigsize / 2), big: true }
          }
          let myLine = 1;
          let lineCircels = 8;
          let check = false;
          ...
          while (check == false) {
              ...
              if (i - calculated < 0) {
                  ...
                  realcircels = (timers.length - calculated) > 0 ? lineCircels : lineCircels + (timers.length - calculated)
                  check = true;
              } else {
                  myLine += 1;
              }
          }
          const angle = ((i-calculated) / realcircels) * 2 * Math.PI
          ...
          return { myline: myLine, lineCircels: realcircels, x, y, big: false };
      }
  
    // Function to fetch timer data
    async function fetchTimers() {
      const userId = $page.params.id; //  Get user ID from the route (adjust as needed)
        if (!userId) {
      ...
      await sendToSer(
        ...
        0,
        true,
        fetch  // Use the built-in fetch
      ).then((res) => {
        if (res && res.usersPermissionsUser && res.usersPermissionsUser.data && res.usersPermissionsUser.data.attributes && res.usersPermissionsUser.data.attributes.mesimabetahaliches) {
            timers = res.usersPermissionsUser.data.attributes.mesimabetahaliches.data.map(t => {
                let totalMilliseconds = 0;
                if (t.attributes.activeTimer && t.attributes.activeTimer.isActive) {
                    totalMilliseconds = t.attributes.activeTimer.totalHours; // Initialize with totalHours
                    ...
                    t.attributes.activeTimer.timers.forEach(timer => {
                        if (timer.start && timer.stop) {
                            totalMilliseconds += new Date(timer.stop).getTime() - new Date(timer.start).getTime();
                              ...
                              const url = t.attributes.project?.data?.attributes?.profilepic?.data?.attributes?.formats?.thumbnails?.url
                const fullUrl =  url
                return{
                    ...t,
          ...
          timers = []; // Ensure timers is an empty array on error
        }
      }).catch(error => {
    ...
    $: {
      center = {x:w/2,y:h/2}
        ...
        console.log(center,"recalcl",timers.length)
    }
    $: rotation = (timer.zman / 1000) * 6;  // 6 degrees per second
$: rotationm = (timer.zman/60000)*6 + ((timer.zman % 60000)/60000)*6; // 6 degrees per minutes.
$: rotationh = (timer.zman / 3600000) * 30 + (((timer.zman % 3_600_000) / 3_600_000) * 30)
  </script>
  
  <div bind:clientWidth={w} bind:clientHeight={h}  class="timer-container">
     <div style="position:absolute; color: white;">{hoverText}</div>
    {#each timers as timer, index (timer.id)}
      {@const myline = orders[index] ?? {x:0,y:0}}
      <svg  class="timer" style="width:{orders[index]?.big? bigsize : size}px; left:{orders[index]?.x}px; top:{orders[index]?.y}px"  viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:bx="https://boxy-svg.com">
              ...
              <g transform="matrix(0.7, 0, 0, 0.7, 0, -44)">
                ...
                <g>
                     ...
                     <foreignObject  x='300' y='105' width='100' height='100' transform='translate(-50 -50)'>
                      <span class="{`normSml${timer.perhour}-${timer.projectId}-${timer.mId}`}"></span>
                          <img on:click={()=>project()} on:keypress={()=>project()} on:mouseenter={()=>hover("לוגו הריקמה")} on:mouseleave={()=>hover("0")} style=" border-radius: 50%;" src={timer.src} width="24" height="24"   alt="logo">
                      </foreignObject>
                          ...
                          <g style="overflow:hidden; text-anchor: middle;" on:click={()=>linke("p")} on:mouseenter={()=>hover("לחיצה כפולה לצפיה בעמוד הציבורי של הריקמה")} on:mouseleave={()=>hover("0")}  >
                                    <text x="300" y='210' fill="#00ffff" style="filter: url(#glow);"  text-anchor="middle" font-size="42" >{timer.projectName}</text>
                                    <text x="300" y='210' fill="black"  text-anchor="middle" font-size="42" >{timer.projectName}</text></g>
                       ...
                       <path transform="matrix(1.4, 0, 0, 1.4 , 20, 280)" fill="transparent" stroke="transparent" id="curveooo8" d=" M 0 0 A 200 200 0 0 0 400 0" />
                              ...
                              <textPath font-size="60" fill="#00ffff" font-weight="bold" stroke="purple" xlink:href="#curveooo8">
                                    <tspan fill="#00ffff" font-weight="bold" stroke="purple" dy="-5">{timer.missionName}</tspan>
                                  </textPath>
                          </text>
                      </svg>
                    {/each}
