...
// לכן הצעה לעולם אינה נשמרת בשקט: היא כבויה כברירת מחדל, ומסך הסקירה מציג
// לצידה את הרשומה שאליה היא תישמר בפועל, כדי שהמשתמש יאשר מיפוי שהוא רואה.
// העלאת הסף כאן עדיין כדאית, אבל דורשת מדידה מול קורפוס — לא ניחוש.
//
// שווה גם לשקול queryNearest עם topK>1: כרגע נשלף מועמד יחיד, ולכן אין דרך
// להציע "התכוונת ל…?" או לזהות שהמועמד היחיד גרוע.
const THRESHOLDS = {
    MATCH: 0.88,
    SUGGESTION: 0.72,
};
