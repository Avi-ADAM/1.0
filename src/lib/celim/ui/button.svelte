<script>
  ...
  function onclick() {
    if (success == false) onClick?.();
  }
  let hover = $state(false);
</script>

<button
  class="cursor-pointer transition-all duration-300 {size === 'sm'
    ? 'px-4 py-1'
...
font-bold font-rubik {className}"
  ...
  ><div class="flex flex-row align-center justify-center items-center gap-4">
    ...
    <h2
      class=" {size === 'sm'
        ? 'text-purple-800 font-litt font-extrabold  text-base md:text-lg'
        : 'text-transparent font-extrabold text-xl md:text-2xl'}  bg-clip-text bg-[length:auto_200%] animate-gradienty transition-all duration-300 tracking-wide
        {hover == false
        ? 'bg-[linear-gradient(to_top,theme(colors.purple.800),theme(colors.pink.800),theme(colors.mpink),theme(colors.pink.400),theme(colors.barbi))]'
