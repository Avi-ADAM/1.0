...
export function registerAction(config: ActionConfig): void {
  ...
  if (actionRegistry.has(config.key)) {
    // In development mode (especially with HMR), re-registration is common.
    // Instead of throwing, we'll update the existing registration.
    console.warn(`Action "${config.key}" is being re-registered.`); // Optional: Keep log
...
function validateActionConfig(config: ActionConfig): void {
  ...
  if (!config.graphqlOperation || (typeof config.graphqlOperation !== 'string' && typeof config.graphqlOperation !== 'function')) {
    throw new Error(`Action "${config.key}" must have a valid "graphqlOperation" field (string or function)`);
  }
  ...
  for (const rule of config.authRules) {
    ...
    if (!['jwt', 'self', 'projectMember', 'sheirutCustomer', 'sheirutpendRequester', 'forumParticipant', 'role', 'custom', 'or'].includes(rule.type)) {
      throw new Error(
