...
export class ActionService {
  ...
  async executeAction(
    ...
    try {
      ...
      // Step 5b: Mirrored-task sync (PLAN_EXTERNAL_TASKS_API).
      // A task that mirrors something — an external system's item, or a 1lev1
      // site report — syncs back when a human accepts, advances or finishes it.
      // Fire-and-forget like notifications: an unreachable receiver must never
      // fail the member's action.
      //
      ...
      if (actionKey === 'createTask' || actionKey === 'updateTask') {
        import('../webhooks/dispatch.js')
          ...
          .catch((err) => {
            this.logger.warn('Mirrored-task sync error', {
              actionKey,
              error: err instanceof Error ? err.message : String(err)
            });
          });
