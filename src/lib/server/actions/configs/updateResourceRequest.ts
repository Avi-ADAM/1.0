...
const handler: ActionExecutionHandler = async (params, context) => {
  ...
  const res = await f(import.meta.env.VITE_URL + '/graphql', {
    ...
    body: JSON.stringify({
      query: `mutation UpdateSp($id: ID!, $data: SpInput!) {
        updateSp(id: $id, data: $data) {
          data { id attributes { name } }
        }
      }`,
      variables: { id, data: spData }
    })
  });
  const json = await res.json();
  if (json.errors) throw new Error(json.errors[0].message);
  return json.data.updateSp.data;
};

export const updateResourceRequestConfig: ActionConfig = {
  key: 'updateResourceRequest',
  description: 'Update a personal resource offering (Sp)',
  graphqlOperation: handler,
  paramSchema: {
    id:      { type: 'string',  required: true,  description: 'Sp ID to update' },
  ...
  authRules: [{ type: 'jwt', errorMessage: 'You must be logged in to update resources' }],
