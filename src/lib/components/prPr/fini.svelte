...
<script>
  ...
  let {
    ...
    meData = $bindable([]),
    users,
    remove = /** @param {any} id */ (id) => {},
    edit = /** @param {any} id */ (id) => {},
    confirm = /** @param {any} id */ (id) => {}
  } = $props();
...
{#if hagdel === false}
    ...
    <div class="yy">
      <Tooltip title="{fir}: {ssec}%">
        <svg
          width="250px"
          height="250px"
          style="display: inline; margin: 20px auto;"
          viewBox="0 0 64 64"
          class="pie"
        >
          {#each ulist as use, i}
            ...
            <circle
              onmouseenter={x('x', use.un, use.p)}
              r="25%"
              cx="50%"
              cy="50%"
              stroke-dasharray="{use.s + 1}, 101"
              stroke-dashoffset={use.d}
              stroke={use.c}
  ...
  <button
    ...
    onclick={() => (hagdel = true)}>פירוט</button
  ><br />
{:else}
  <button
    title={cl[$lang]}
