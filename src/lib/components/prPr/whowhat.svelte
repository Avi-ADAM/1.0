<script>
  ...
  async function ask() {
    ...
    try {
      ...
      // Off by default (SITE_SHARE_FLAGS.manualSplit). When on, the treasury
      // recipient is resolved from the platform project (isPlatform=true) unless
      // a static override is set, and the platform never charges its own split.
      // In a manual split all members are providers, so the basis is the
      // distributed profit (revach). See docs/PLAN_SITE_SHARE.md §2, §6.1.
      if (SITE_SHARE_FLAGS.manualSplit) {
        let treasuryUserId = SITE_SHARE_TREASURY_USER_ID;
        let platformProjectId = SITE_SHARE_PLATFORM_PROJECT_ID;
        if (!treasuryUserId) {
          const pp = await executeAction('getPlatformProject', {});
          if (pp.success && pp.data?.configured) {
            treasuryUserId = String(pp.data.treasuryUserId);
            platformProjectId = String(pp.data.projectId);
          }
        }
        const isPlatformOwnSplit =
          platformProjectId && String($idPr) === String(platformProjectId);
        const siteLines = isPlatformOwnSplit
          ? null
          : buildSiteShareLines({
              enabled: true,
              payerRole: 'provider',
