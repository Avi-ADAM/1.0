...
onMount(async () => {
    for (let i = 0; i < miData.length; i++) {
        if (miData[i].selectedSkills.length == 0) {
            miData[i].selectedSkills = miData[i].attributes.skills.data.map(c => c.attributes.skillName)
        }
        if (miData[i].selectedRoles.length == 0) {
            miData[i].selectedRoles = miData[i].attributes.tafkidims.data.map(c => c.attributes.roleDescription)
        }
    }
    if (restime == "feh") {
        x = 48 * 60 * 60 * 1000
...
const iskvua = {
    "he": "האם זו משרה קבועה",
    "en": "is that a constant job"
};
const iskvu = {
    "he": "משרה קבועה",
    "en": "constant job"
...
function shifterr(o) {
    ...
    for (let i = 0; i < days.length; i++) {
        if (days[i].shiftp > shifts) {
            ...
            shift = [...Array(shifts).fill(0).map(x => ({
                ii: ''
            }))]
...
<DialogOverlay style="z-index: 700;" {isOpen} onDismiss={closer} >
            ...
            <div style="z-index: 400; overflow-x: auto;" dir="{$lang == "he" ? "rtl" : "ltr"}" class="d" >
                <button class=" hover:bg-barbi text-mturk rounded-full"
                    on:click={closer}><Close/>
                        ...
                        <table dir="{$lang == "he" ? "rtl" : "ltr"}" class="d" style="overflow-x: auto; font-size: 95%;">
                            ...
                            <div class="body items-center d">
                                ...
                                <table dir="{$lang == "he" ? "rtl" : "ltr"}" >
                                    ...
                                    <tr class="gg">
                                        ...
                                        <th>{requireSkills[$lang]}</th>
                                        {#each miData as data, i}
                                        <td class=" max-w-md"> <MultiSelect
                                            loading={newcontent}
                                            on:change={() => mi.set(miData)}
                                            bind:selected={data.selectedSkills}
                                            placeholder={placeholder1[$lang]}
                                            options={$skil.map(c => c.attributes.skillName)}
                                            noMatchingOptionsMsg={nom[$lang]}
                                            />
                                            <AddNewSkill color={"--barbi-pink"} mid={data.id} on:addnewskill={addnew} addS={addS} roles1={roles} />
                                            </td>
                                        {/each}
                                            </tr>
                                            <tr>
                                                <th>{requiredRoles[$lang]}</th>
                                                {#each miData as data, i}
                                                <td> <MultiSelect
                                                    loading={newcontentR}
                                                    bind:selected={data.selectedRoles}
                                                    on:change={() => mi.set(miData)}
  ...
  <td><MultiSelect
      ...
      bind:searchText
      loading={newcontentW}
      bind:selected={data.selectedWorkways}
      {placeholder}
      options={$ww.map(c => c.attributes.workWayName)}
      on:change={(e) => {addW(data.selectedWorkways, data.id,e)}}
      />
...
.input:focus~.line,
.input:valid~.line,
#inputii:valid~#labelii,
#inputii:focus~#labelii {
    width: 100%;
}
