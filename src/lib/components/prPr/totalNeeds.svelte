...
<<<<<<< HEAD
    for (const element of meData) {
      let d = new Date();
      const spnot =
        element.spnot !== undefined ? addslashes(element.spnot) : '';
      ...
      const sdate =
        element.attributes.dates !== undefined
          ? `sqadualed: "${new Date(element.attributes.dates).toISOString()}",`
          : ``;
      const fdate =
        element.attributes.datef !== undefined
          ? ` sqadualedf: "${new Date(element.attributes.datef).toISOString()}" ,`
          : ``;
...
<<<<<<< HEAD
    ...
    if (meData[i].attributes.kindOf === 'monthly') {
      ...
      meData[i].ky = true;
      meData[i].kc = false;
      meData[i].total = (meData[i].years * meData[i].attributes.price).toFixed(
        2
      );
      meData[i].totaltotal = (
          ...
          <tr>
            ...
            {#each meData as data, i}
              <td>
                ...
                {#if data.attributes.price < 0}<small
    ...
    <div>
      {#if already === false}
        <button
          ...
          onclick={han}>פרסום משאבים נדרשים</button
        >
      {:else}
