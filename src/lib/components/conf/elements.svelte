...
<div class="border border-gold border-opacity-20 rounded m-2">
          ...
          {#each dati as dat, i}
            <p class="m-0" style="text-shadow:none; white-space:none;">
              <Tile
                sm={true}
                big={true}
                bg={dat.added ? 'green' : dat.remuved ? 'red' : bgi}
                closei={dat.remuved}
                openi={dat.added}
  ...
  {:else}
    <div
      class="max-w-md mx-auto flex flex-row gap-x-3 justify-center align-middle"
    >
      <MultiSelect
      ...
      <div class="mx-auto">
        ...
        {:else if valc == 'roleDescription'}
          <AddNewRole color={'--barbi-pink'} addnewrole={addnew} />
        {:else if valc == 'workWayName'}
          <AddnewWorkway color={'--barbi-pink'} addww={addnew} />
        {/if}
      </div>
      <button
        onclick={() => {
