<script lang="ts">
  ...
  const texts = {
    he: {
      ...
      intro: `הגיע הזמן לעדכן את ההוצאה החודשית עבור "${resourceName}"${projectName ? ` ברקמה "${projectName}"` : ''} לחודש ${monthLabel}.`,
      plannedLabel: 'הסכום המתוכנן',
      ask: 'כנסו לעמוד הלב, אשרו או עדכנו את הסכום שהוצאתם בפועל החודש, וזה יעבור לאישור הרקמה ויירשם בארכיון.',
    ...
    ru: {
      title: 'Повторяющийся ресурс активен 🔁',
      greeting: `Здравствуйте, ${username},`,
      intro: `Пора записать расходы этого месяца за «${resourceName}»${projectName ? ` в плетении «${projectName}»` : ''} за ${monthLabel}.`,
      plannedLabel: 'Запланированная сумма',
      ask: 'Перейдите на страницу Лев, подтвердите или обновите фактически потраченную сумму в этом месяце - она уйдёт на согласование и будет записана в архив.',
      cta: 'Обновить расход в Лев',
