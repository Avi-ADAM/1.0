<script>
	...
	const previewText = {
		he: `${un} מהרקמה ${pn} חשב/ה בדיוק עליך - "${name}" מחכה לאישורך`,
	...
	const opening = {
		he: `ברקמה "${pn}" נולדה משימה חדשה - "${name}" - ו${un} בחר/ה דווקא בך להוביל אותה. הכישורים שלך, הדרך שלך - זה בדיוק מה שההצעה הזו מחפשת.`,
	...
	const consent = {
		he: `וחשוב מכל: שום דבר לא קורה בלי ההסכמה שלך. המשימה תירשם על שמך רק אם תאשר/י אותה. לרשותך ${restimeTxt.he} לאשר, לדייק את התנאים בהצעה נגדית או לפתוח שיחה - ואם לא תגיב/י עד אז, ההצעה תיפתח לכל חברי הרקמה והיא תחפש מישהו אחר.`,
