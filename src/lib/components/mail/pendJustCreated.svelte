<script >
	...
	const mainText = {"he":`ברקמה ${pn} ${hoza[kind]} "${name}" על ידי ${un} ${kind == "pend" || kind == "pendAsk" ? "והיא עומדת להצבעה!" : kind == "pendmash" ? "והוא עומד להצבעה!" : "והיא עכשיו בתהליך אישור"} `,
                    "en":`in the rikma ${pn} ${kind == "pend" || kind == "pendAsk" ? "a new mission named" : kind == "pendmash" ? "a new resorce named" : "the mission"} "${name}" hes ${kind == "finiappmi" ? "complited sucssesfully" : "suggested"} by ${un} and the appruval vote just started!`}
