...
function find_contry_id(contry_name_arr){
  ...
  chezins { 
   ...
   meta {
    ...
    let selected = $state($state($state($state($state($state($state($state([]))))))));
       let already = $state($state($state($state($state($state($state($state(false))))))));
   let datar;
   let idx = $state($state($state($state($state($state($state($state(1))))))));
   let data;
...
}let trans = $state($state($state($state($state($state($state($state(false))))))));
function tran (){
trans = !trans;
}
let error;
...
onMount(async () => {
 ...
 idx = res + 2
      ...
      let dow = $state($state($state($state($state($state($state($state())))))));
    function scrollTo() {
		dow.scrollIntoView({ behavior: 'smooth' });
	}
     
 let isOpen = $state($state($state($state($state($state($state($state(false))))))));
let a = $state($state($state($state($state($state($state($state(0))))))));
