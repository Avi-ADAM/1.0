<script>
  ...
  const country = [
    ...
    { value: 4, label: 'Anguilla', heb: 'אנגווילה', ar: 'أنجويلا' },
    {
      ...
      label: 'Antarctica',
      heb: 'אנטרטיקה',
      ar: 'المنطقة القطبية الجنوبية'
    },
    {
      value: 6,
      label: 'Antigua and Barbuda',
      heb: 'אנטיגואה וברבודה',
      ar: 'أنتيغوا وباربودا	'
    },
      ...
      <div style="z-index: 400;" dir="rtl">
        ...
        {:else if a == 1}
          <div class="sp bg-gold">
            <h3 class="text-barbi">
              تم الإرسال بنجاح ، شكرًا جزيلاً سنتواصل معك
            </h3>
          </div>
        {:else if a == 2}
        ...
        {:else if a == 3}
          ...
          <button
            ...
            onclick={() => (a = 0)}>أعد المحاولة</button
          >
        {:else if a == 5}
          <h1>حدث خطأ</h1>
          <button
            class="hover:bg-barbi text-barbi hover:text-gold bg-gold rounded-full"
            onclick={() => (a = 4)}>أعد المحاولة</button
          >
        {/if}
      </div></DialogContent
    >
...
<div class="all">
  ...
  >
    ...
    {:else}
      ...
      <!--todo: to home-->
      <button
        style="border-bottom-width: 4px; border-color: var(--gold);"
        class="text-barbi text-bold hover:text-lturk bg-lturk text-center hover:bg-barbi px-1 py-0.5"
        onclick={() => change('en')}>English</button
      >
      <button
        class="text-barbi text-bold hover:text-lturk text-center bg-lturk hover:bg-barbi px-1 py-0.5"
        onclick={() => change('he')}>עברית</button
      >
      <button
        onclick={sell}
        title=" اطلب تغيير النص"
        class="text-barbi border-2 border-gold text-bold hover:text-lturk bg-lturk text-center hover:bg-barbi px-1 py-0.5"
        >اطلب تغيير النص</button
      >
      <button
        onclick={tr}
        title="الترجمة إلى لغات أخرى"
        class="text-barbi border-2 border-gold text-bold hover:text-lturk bg-lturk text-center hover:bg-barbi px-1 py-0.5"
        >ترجم</button
      >
      <button
    ...
    <section class="container" dir="rtl" id="lines">
      ...
      <div class="flexi1">
        <h3
          class="amanat"
          id="m"
          style="font-family: StamSefarad, serif; font-size: 1em;"
          dir="rtl"
        >
          من:
        </h3>
          ...
          >
            <span style="font-family:David;">
              أنا <span
                ...
                >{$form.name ? $form.name : '__'}</span
              >
              لا أريد أن أؤذي أي شخص ولن أؤذي أي شخص أبدًا.
              <br />
              لأنني أعتقد أنه ليس هناك سلطة أو قيمة أو هدف أو إيمان أو مال أو مصلحة
              يمكن أن يبرر الإيذاء البشري أو العنف أو القوة.
              <br />
              أنا أؤمن بالخير الأساسي في الإنسان، وأتمنى وأتوقع أنه عندما يوافق جميع
              البشر على عدم العنف والحروب واستخدام القوة، سيتوقفون عن أن يكونوا وسيلة
              للتواصل البشري.
              <br />
              عندما يوافق جميع سكان
              <span
              ...
              <span
                style="color:black; font-family:StamSefarad;  text-shadow: 1px 1px var(--barbi-pink);"
                >{selected.length > 0
                  ? `${selected.length < 2 ? selected : selected.join(' ومن دولة ')}`
                  : '__'}</span
              >
              {selected.length > 1 ? 'بأسمي' : 'باسمها'} وسنعيش بحرية وبموافقة متبادلة.
              <br />
              عندما يوافق جميع البشرية وتعيش وفقًا لهذا الإيمان، سأتخلى عن أسلحتي
              الخاصة بجيش
              <span
                style="color: black; font-family:StamSefarad;  text-shadow: 1px 1px var(--barbi-pink);"
                >{selected.length > 0
                  ? `${selected.length < 2 ? selected : selected.join(' وجيش ')}` +
                    '.'
                  : '__.'}</span
              >
              عندما تتخلى جميع جيوش العالم عن أسلحتها وتصبح البشرية ضعيفة وحرة
              <br />
              سأظل دائمًا أحل النزاعات وأديرها وأحلها في "رقميات" التي تجري على موقع
              1💗1 فقط بموافقة متبادلة.
            </span>
          </span>
        </div>
      </div>
    ...
    <form onsubmit={handleSubmit}>
      <div class="flexid">
        ...
        {:else if already == true}
          <h1 class="alredy" dir="rtl">
            {$form.name}
            تم استلام توقيعك ، لقد وصلت إلى الرقم {idx} ، سيتم إرسال بريد إلكتروني
            للتحديث عندما نتوسع ، قريبًا
          </h1>
          <h2>
            The site other sections are not yet in arabic, you been transformed
            to the english site
          </h2>
