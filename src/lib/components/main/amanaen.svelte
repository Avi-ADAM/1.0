<script>
  ...
  let erorim = $state({
    st: false,
    msg: '',
    msg2: 'if the problem continues please contact us at',
        ...
        if (selected.length < 1) {
          ...
          axios
            ...
            .then((response) => {
              ...
              already = true;
              document.cookie =
                `email=${mail}; expires=` + new Date(2024, 0, 1).toUTCString();
              document.cookie =
                `un=${$form.name}; expires=` +
                new Date(2024, 0, 1).toUTCString();
...
<div class="all">
  ...
  <div
    style="position:absolute ; left: 1%; top: 1%; display: flex; flex-direction: column ;"
  >
    {#if trans === false}
      <button onclick={tran}
        ><img
          alt="translation-by-bar-sultan"
    ...
    {:else}
      ...
      <button
        onclick={() => change('he')}
        title="למעבר לשפה העברית"
        class="text-barbi border-2 border-gold text-bold hover:text-lturk bg-lturk text-center hover:bg-barbi px-1 py-0.5"
        >עברית</button
      >
      <button
        class="text-barbi text-bold hover:text-lturk text-center bg-lturk hover:bg-barbi px-1 py-0.5"
        onclick={() => change('ar')}>العربية</button
      >
      <button
        onclick={sell}
        title="ask for change in the text"
        class="text-barbi border-2 border-gold text-bold hover:text-lturk bg-lturk text-center hover:bg-barbi px-1 py-0.5"
        >suggest text change</button
      >
      <button
        onclick={tr}
  ...
  <div class="mobile">
    <section class="container" dir="ltr" id="lines">
      <div class="flexi">
        ...
        />
        {#if $errors.name}
          <small style="color: red; white-space: pre-wrap;"
            >{$errors.name}</small
          >
        {/if}
      </div>
      <div class="flexi1">
        <h3
          class="amanat"
          id="m"
          style="font-family: StamSefarad, serif; font-size: 1em; font-weight: 700;"
          dir="ltr"
        >
          from:
        </h3>
      ...
      <div class="flexi2">
        <h3
          class="amanat"
          style="font-weight: 700; white-space: nowrap; font-family: 'StamSefarad', serif; font-size: 1em; line-height:normal;"
          dir="ltr"
        >
          Email:
        </h3>
    ...
    <div dir="ltr" class="amana" id="amana-show">
      ...
      >
        ...
        <div class="card-inner d overflow-y-auto">
          <h1
            dir="ltr"
            style="color:var(--barbi-pink); text-decoration: underline; font-weight: 900;"
          >
            <span
              style="text-shadow: 1px 1px var(--mturk); font-family: 'Gan','Rubik';"
              >{$form.name ? $form.name : '__'}</span
          ...
          >
            <span style="font-family:poppins,Gan;">
              I, <span style="text-shadow: 1px 1px var(--mturk);"
                >{$form.name ? $form.name : '__'}</span
              >, will never use violence or harm anyone.
              <br />
              Because I,
              <span style="text-shadow: 1px 1px var(--mturk);"
                >{$form.name ? $form.name : '__'}</span
              >, do not want to be a victim of violence, and because there is no
              authority, value, purpose, faith, money, or interest that
              justifies harming a person's life through violence or coercion.
              <br />
              I trust in the inherent goodness of humanity, and I hope that when all
              of humanity signs this Convention, violence, conflict, and coercion
              will cease to be forms of human communication.
              <br />
              When the entire
              <span
                style="color: black; text-shadow: 1px 1px var(--barbi-pink);"
                >{selected.length > 0 ? selected : '__'}</span
              >
              becomes a signatory to this Convention, I,
              <span style="text-shadow: 1px 1px var(--mturk);"
                >{$form.name ? $form.name : '__'}</span
              >, will relinquish my weapons and the armed policemen acting on
              behalf of the
              <span
                style="color: black; text-shadow: 1px 1px var(--barbi-pink);"
                >{selected.length > 0 ? selected : '__'}</span
              ...
              State.
              <br />
              I will also give up the weapons of the
              <span
                style="color: black; text-shadow: 1px 1px var(--barbi-pink);"
                >{selected.length > 0 ? selected : '__'}</span
              >
              Army when all of humanity becomes a signatory to this Convention.
              <br />
              Furthermore, I commit to creating, managing, and resolving disagreements
              on the 1💗1 platform through mutual agreement.
            </span>
          </span>
        </div>
      </div>
