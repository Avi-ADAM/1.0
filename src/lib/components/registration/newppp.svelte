 ...
 let uid = cookieValueId;
  ...
  .then(data => miDatan = data);
   ...
   dispatch ('progres',{
    ...
    document.cookie = `jwt=${miDatan.data.jwt}; expires=` + new Date(2025, 0, 1).toUTCString();
    document.cookie = `id=${miDatan.data.user.id}; expires=` + new Date(2025, 0, 1).toUTCString();
} catch (e) {
            error1 = e
            console.log(error1);
