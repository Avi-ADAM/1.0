...
$effect(() => {
	...
	chan = changed;
	console.log('Changed:', changed, { un, bi, githublink, twiterlink, discordlink, fblink, frd, lango, checked, noMail });
...
githublink,
	...
	async function toggleGuide(show) {
		...
		try {
			const q = `mutation { updateUsersPermissionsUser(id:${uid}, data: { profilManualAlready: ${!show} }) { data {id} } }`;
			// await SendTo(q);
			console.log('Sending guide status to server');
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5">
		...
		<Card>
			...
			<CardContent class="space-y-4 ">
				...
				<div>
					<label for="bio" class="block text-gold text-sm font-medium mb-1">{t[$lang].bio}</label>
					<textarea
						id="bio"
						bind:value={bi}
		...
		<Card>
			...
			<CardContent class="space-y-4">
				<TextInput lebel={{ en: 'GitHub', he: 'גיטהאב' }} bind:text={githublink} />
				<TextInput lebel={{ en: 'Twitter', he: 'טוויטר' }} bind:text={twiterlink} />
				<TextInput lebel={{ en: 'Discord', he: 'דיסקורד' }} bind:text={discordlink} />
				<TextInput lebel={{ en: 'Facebook', he: 'פייסבוק' }} bind:text={fblink} />
			</CardContent>
		</Card>
		...
		<Card>
			...
			<CardContent class="space-y-6">
				<div>
					<label for="free-day" class="block text-sm font-medium mb-1 text-gold">{t[$lang].freeDay}</label>
					<select
						id="free-day"
						bind:value={frd}
		...
		<Card>
			...
			<CardContent class="space-y-4">
				...
				<div>
					...
					<LoginT {uid} requestAccess={true} username="onelev_bot" onOuth={() => (teleredy = true)} />
				</div>
				<Separator />
				<Checkbox name="noMail" lebel={noEmails} lang={$lang} bind:value={noMail} />
				<Separator />
					{#if !changePassword}
				<Button
...
-->
		...
		{#if chan}
			<Button text={{ he: t.he.save, en: t.en.save }} onClick={save} disabled={!chan} />
		{/if}	
</div>
