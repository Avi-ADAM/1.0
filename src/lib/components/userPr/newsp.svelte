<script>
  ...
  let token;
  let { needr = [], meData = [], remove, close } = $props();
  let miDatan = [];
    ...
    for (const element of meData) {
      ...
      const sdate =
        element.dates !== undefined
          ? `sdate: "${new Date(element.dates).toISOString()}",`
          : ``;
      const fdate =
        element.datef !== undefined
          ? `fdate: "${new Date(element.datef).toISOString()}" ,`
          : ``;
