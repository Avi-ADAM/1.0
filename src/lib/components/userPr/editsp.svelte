<script>
  ...
  function myMi() {
    ...
    meData.dates !== undefined || null || 0
      ? meData.dates
      : new Date().toISOString().slice(0, -1);
    meData.datef !== undefined || null || 0
      ? meData.datef
  ...
  async function han() {
    ...
    const sdate =
      meData.dates !== undefined
        ? `sdate: "${new Date(meData.dates).toISOString()}",`
        : ``;
    const fdate =
      meData.datef !== undefined
        ? `fdate: "${new Date(meData.datef).toISOString()}" ,`
        : ``;
