<script>
  ...
  async function submit() {
    ...
    try {
      ...
      toast.success(
        res?.data?.immediate ? $t('archive.edit.applied') : $t('archive.edit.proposed')
      );
      open = false;
      onDone?.();
    } catch (e) {
  ...
  <Drawer.Portal>
    ...
    >
      ...
      <div class="overflow-y-auto px-5 pb-6 space-y-5">
        ...
        <label class="block">
          <span class="text-sm text-gray-600 dark:text-gray-400">{$t('archive.propose.why')}</span>
          <textarea
            rows="3"
            bind:value={why}
        ...
        <p class="text-xs text-gray-500">
          {soleMember ? $t('archive.propose.immediateNote') : $t('archive.edit.note')}
        </p>
