<script>
  ...
  data { id attributes{ projectName}}
        ...
        let data = {
          ...
          det: `${projectName_value} והתיאור: ${desP}`
        };
        fetch('/api/ste', {
          method: 'POST', // or 'PUT'
          headers: {
  ...
  const hre = {
    he: 'זמן תגובה לקבלת החלטות בריקמה',
    en: 'time to respond to FreeMates voting'
  };
  const teure = {
    he: 'תיאור שיהיה גלוי לכל',
    en: 'short description with public visibility'
  };
  const prte = {
    he: 'תאור מפורט שגלוי רק בתוך הריקמה',
    en: 'long description visible only to the FreeMates members'
  };
  ...
  const whva = {
    he: 'אלו ערכים ומטרות הריקמה תקדם',
    en: 'which vallues and goals the FreeMates will promote'
  };
  ...
  const tob = {
    he: 'מעבר לניהול הריקמה במוח הריקמה',
    en: 'to the FreeMates brain'
  };
  const inc = {
    he: 'ניתן להזין את הערך המוערך של ההכנסה אם ידוע, אחרת ניתן יהיה לחשב בהמשך מדף הניהול',
    en: 'if you know the aproximate vallue of income, else you can later caculate it'
  };
</script>
...
{#if before == false}
 ...
 type='text' class='input d' required></textarea>
  ...
  <span class='line'></span>
    ...
    <button
      onclick={openen}
      class="border border-barbi hover:border-gold bg-gradient-to-br from-gra via-grb via-gr-c via-grd to-gre hover:from-barbi hover:to-mpink text-barbi hover:text-gold rounded-full p-2"
      >{ladd[$lang]}</button
    >
    ...
    <Chooser
      tr={{ he: 'ריקמה מתמשכת', en: 'continuous' }}
      fl={{ he: 'ריקמה חד פעמית', en: 'one timed' }}
      level={{
        he: 'הריקמה מיועדת להיות אירוע חד פעמי או מתמשך? לדוגמה: הפקת אירוע או הקמת עסק להפקת אירועים',
