...
function dispatchb () {
  ...
  /** @type {{roles1?: any, mid?: any, rn?: any, addS?: boolean, color?: string}} */
  let {
    b,
    addnewskill,
    roles1 = $bindable([]),
...
onclick={() => addro = false}
 ...
 ><svg style="width:24px;height:24px" viewBox="0 0 24 24">
  ...
  <Addnewro {color} rn={roles1.map(c => c.roleDescription)} {finnish}/>
  {/if}
</div>
