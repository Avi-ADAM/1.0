<script>
  ...
  let unitWord = $derived(
    kindOf === 'yearly'
      ? {
          per: $t('lev.cards.common.perYear'),
          many: $t('lev.cards.common.years'),
          one: $t('lev.cards.common.year')
        }
      : {
          per: $t('lev.cards.common.perMonth'),
          many: $t('lev.cards.common.months'),
          one: $t('lev.cards.common.month')
        }
  );
  ...
  // Open-ended recurring resource (no end date ⇒ no recurTotal): the equity
  // value above is a single cycle, so hand EquityPreview the ₪/month so it can
  // project 1/2/5 years instead of pricing one cycle forever.
  let equityMonthlyValue = $derived(
    recurring && cycleCount == null && perCycle > 0
      ? perCycle / (Math.max(1, Number(cycleSize) || 1) * (kindOf === 'yearly' ? 12 : 1))
      : null
  );
</script>
  ...
  >
          ...
          >
            ...
            >
              ...
              {#if myRound.price != null}
                · {myRound.price?.toLocaleString?.() ?? myRound.price}
                {$t('lev.cards.sugestma.acceptedVal')}
              {/if}
            </span>
    ...
    <div class="flex flex-col space-y-4">
      {#if recurring}
        ...
        >
          <div class="flex items-center flex-wrap gap-2">
            ...
            <span
              role="contentinfo"
              onmouseenter={() => hover($t('lev.cards.sugestma.perCycleHint'))}
            ...
            {#if cycleSize > 1}
              <span class="text-sm text-gray-600 dark:text-gray-300">
                {kindOf === 'yearly'
                  ? $t('lev.cards.sugestma.everyNYears', { count: cycleSize })
                  : $t('lev.cards.sugestma.everyNMonths', { count: cycleSize })}
              </span>
            {/if}
          </div>
          {#if recurTotal != null}
            <div
              role="contentinfo"
              onmouseenter={() => hover($t('lev.cards.sugestma.totalInvestHint'))}
              onmouseleave={() => hover('0')}
              class="text-sm text-gray-700 dark:text-gray-200"
            >
              {$t('lev.cards.sugestma.totalInvest')}:
          ...
          {:else}
            <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">
              ♾️ {$t('lev.cards.sugestma.noEndDate')}
            </div>
          {/if}
          <div class="text-xs text-gray-500 dark:text-gray-400">
            {$t('lev.cards.sugestma.cycleApprovalNote')}
          </div>
        </div>
      {/if}
      <div
      ...
      <div class="flex items-center gap-3">
        <h3
          onmouseenter={() => hover($t('lev.cards.sugestma.value'))}
          onmouseleave={() => hover('0')}
          class="text-lg font-bold text-gray-900 dark:text-gray-100"
        >
          {price}
          <span class="text-sm font-normal text-gray-500 dark:text-gray-400"
            >{$t('lev.cards.sugestma.acceptedVal')}</span
          >
        </h3>
        {#if total}
  ...
  >
    {#if low == false}
      {#if offerHref}
        ...
        <button
          onmouseenter={() => hover($t('lev.cards.decline'))}
          onmouseleave={() => hover('0')}
      ...
      {:else if already === false}
        ...
        >
          ...
          <span class="whitespace-nowrap"
            >{$t('lev.cards.common.counterProposal')}</span
          >
        </button>
        <button
