...
{:else if buble.ani === "meData" && milon.sugg == true}
 ...
 on:user={user}
    ...
    {low}
      ...
      oid = {buble.id}
              projectName = {buble.attributes.projectName}
                role ={buble.attributes.tafkidims.data}
              skills ={ buble.attributes.skills.data} 
              missionDetails = {buble.descrip} 
              notes = {buble.attributes.hearotMeyuchadot}
           src = {buble.attributes.project.data.attributes.profilePic.data?.attributes.formats.thumbnail.url}//add replasment pic
               missionName={buble.attributes.name}
                projectId={buble.attributes.project.data.id}
                workways={buble.attributes.work_ways.data}
