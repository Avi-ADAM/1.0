...
function opentask(){
 ...
 let eve = $derived(dueDateOrCountToDedline != "undefined" &&  dueDateOrCountToDedline != undefined &&  dueDateOrCountToDedline != null ? new Date(dueDateOrCountToDedline) : null);
...
<div dir={$lang == "he" ? "rtl" : "ltr"}  style="overflow-y:auto" class=" d {isVisible ? $lang == 'he' ? 'boxleft' : 'boxright' : ''} pb-16 leading-normal w-[90%] h-[90%] bg-white lg:w-[90%]">
 <!-- <div class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden bg-gold" style:background-image={`url('${src2}')`} title="">
  ...
  class="de border rounded-2xl border-barbi hover:border-gold mt-3" onclick={()=>statusi()}
    onkeypress={()=>statusi()}
...
<button onmouseenter={()=>hover("לחיצה לאיפוס הטיימר מבלי לשמור")} onmouseleave={()=>hover("0")} 
    class="border border-barbi hover:border-gold bg-gradient-to-br from-graa to-grab text-barbi hover:text-gold  px-4 py-1 rounded hover:from-lturk hover:to-barbi " onclick={clear}>ניקוי</button>
<button onmouseenter={()=>hover("לחיצה לעצירת הטיימר ושמירת הזמן שבוצע")} onmouseleave={()=>hover("0")} 
   class="  bg-gradient-to-br text-gold hover:text-barbi hover:from-graa hover:to-grab 
      px-4 py-1 rounded from-lturk to-barbi " onclick={save}> הוספה</button>
    </div>{/if}
    {#if already === false}
    <button onmouseenter={()=>hover("לחיצה לסיום המשימה")} onmouseleave={()=>hover("0")} onclick={done}  
     ...
     {#if show === true} 
      <button
       onmouseenter={()=>hover(`${running ? "עצירת הטיימר" : "הפעלת טיימר"}`)} 
 ...
 {:else if low == true}
          <Lowbtn isCart="true"/>
        {/if}
 <a onmouseenter={()=>hover(linkDescription)} onmouseleave={()=>hover("0")} 
