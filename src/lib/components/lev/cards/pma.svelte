...
$effect(() => {
  ...
  const previousOffer = nego_mashes && nego_mashes.length > 0 ? nego_mashes[nego_mashes.length - 1]?.attributes : null;
...
<div   onclick={toggleScrollable}
         ...
         <div class="flex flex-col leading-tight">
            <div class=" text-xl mt-1 flex items-center">
               <span class="text-barbi text-center mr-3 sm:text-xl "> 
                 {hasUpdatedOffer ? " הצבעה על הצעה מתוקנת למשאב חדש" : "הצבעה על אישור משאב חדש"}
               </span>
            </div>
            <span style=" text-shadow: 1px 1px white;" class="pn ml-1 text-xl text-barbi ">{projectName}</span>
         </div>
         </div>
         
         {#if timeLeft > 0}
         <div class="flex flex-col items-center m-2">
           <div class="text-xs text-gray-600">אישור אוטומטי בעוד:</div>
           <div class="text-lg font-bold text-red-600 bg-yellow-100 px-2 py-1 rounded">
  ...
  <div  class="{isScrolable.value ? "bg-white" : "bg-gray-200"} transition-all-300 rounded-b lg:rounded-b-none lg:rounded-r p-4 mb-12 flex flex-col justify-between leading-normal">
    ...
    <div  class="mb-8">
     ...
     {#if kindOf === "perUnit"}
       <span onmouseenter={()=>hover(" שווי ליחידה")} onmouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >
         {easy > 0 ? easy.toLocaleString() : price.toLocaleString()} ליחידה
       </span> * <span onmouseenter={()=>hover("כמות")} onmouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >
         {hm == 1 ? "יחידה אחת" : `${hm.toLocaleString()} יחידות`}
       </span> = <span onmouseenter={()=>hover("סך הכל")} onmouseleave={()=>hover("0")} >
         {easy > 0 ? (easy * hm).toLocaleString() : (price * hm).toLocaleString()}
       </span>
   {:else if kindOf === "total" || kindOf === "rent"}
       <span onmouseenter={()=>hover("שווי מוצע")} onmouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >
         {easy > 0 ? easy.toLocaleString() : price.toLocaleString()}
       </span>
   {:else if kindOf === "monthly"}
       <span onmouseenter={()=>hover("שווי לחודש")} onmouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >
         {easy > 0 ? easy.toLocaleString() : price.toLocaleString()} לחודש
       </span> * <span onmouseenter={()=>hover("כמות חודשים")} onmouseleave={()=>hover("0")}  style="color: var(--barbi-pink)" >
         {monts == 1 ? "חודש אחד" : `${monts.toLocaleString()} חודשים`}
       </span> = <span onmouseenter={()=>hover("סך הכל")} onmouseleave={()=>hover("0")} >
         {easy > 0 ? (easy * monts).toLocaleString() : (price * monts).toLocaleString()}
       </span> 
   {:else if kindOf === "yearly"}
       <span onmouseenter={()=>hover("שווי לשנה")} onmouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >
         {easy > 0 ? easy.toLocaleString() : (price).toLocaleString()} לשנה
       </span> * <span onmouseenter={()=>hover("מספר שנים")} onmouseleave={()=>hover("0")}  style="color: var(--barbi-pink)" >
         {yers === 1 ? "שנה אחת" : `${yers.toLocaleString()} שנים`}
       </span> = <span onmouseenter={()=>hover("סך הכל")} onmouseleave={()=>hover("0")} >
         {easy > 0 ? (easy * yers).toLocaleString() : (price * yers).toLocaleString()}
       </span> 
{/if}
        </p>
