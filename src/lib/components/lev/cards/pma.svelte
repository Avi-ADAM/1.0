 ...
 <!-- <div class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden bg-gold" style:background-image={`url('${src2}')`} title="">
  ...
  <div  class="{isScrolable ? "bg-white" : "bg-gray-200"} transition-all-300 rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
    <div  class="mb-8">
     ...
     {#if kindOf === "perUnit"}
       <span onmouseenter={()=>hover(" שווי ליחידה")} onmouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >{easy > 0 ? easy.toLocaleString() : price.toLocaleString()} ליחידה</span> * <span onmouseenter={()=>hover("כמות")} onmouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >{hm == 1 ? "יחידה אחת" : `${hm.toLocaleString()} יחידות`} </span> = <span onmouseenter={()=>hover("סך הכל")} onmouseleave={()=>hover("0")} >{easy > 0 ? (easy * hm).toLocaleString() : (price * hm).toLocaleString()}</span>
   {:else if kindOf === "total" || kindOf === "rent"}
       <span onmouseenter={()=>hover("שווי מוצע")} onmouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >{easy > 0 ? easy.toLocaleString() : price.toLocaleString()}</span>
          {:else if kindOf === "monthly"}
       <span onmouseenter={()=>hover("שווי לחודש")} onmouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >{easy > 0 ? easy.toLocaleString() : price.toLocaleString()} לחודש</span> * <span onmouseenter={()=>hover("כמות חודשים")} onmouseleave={()=>hover("0")}  style="color: var(--barbi-pink)" >{monts == 1 ? "חודש אחד" : `${monts.toLocaleString()} חודשים`}</span> = <span onmouseenter={()=>hover("סך הכל")} onmouseleave={()=>hover("0")} >{easy > 0 ? (easy * monts).toLocaleString() : (price * monts).toLocaleString()}</span> 
          {:else if kindOf === "yearly"}
       <span onmouseenter={()=>hover("שווי לשנה")} onmouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >{easy > 0 ? easy.toLocaleString() : (price).toLocaleString()} לשנה</span> * <span onmouseenter={()=>hover("מספר שנים")} onmouseleave={()=>hover("0")}  style="color: var(--barbi-pink)" >{yers === 1 ? "שנה אחת" : `${yers.toLocaleString()} שנים`}</span> = <span onmouseenter={()=>hover("סך הכל")} onmouseleave={()=>hover("0")} >{easy > 0 ? (easy * yers).toLocaleString() : (price * yers).toLocaleString()}</span> 
{/if}
        </p>
    ...
    {#if hearotMeyuchadot}
     <p onmouseenter={()=>hover("הערות")} onmouseleave={()=>hover("0")} class="text-grey-700 max-h-16 cd  d">{hearotMeyuchadot !== undefined && hearotMeyuchadot !== null && hearotMeyuchadot !== "undefined" ? hearotMeyuchadot : ""}</p>
     {/if} 
    </div>
    <div class="flex items-center">
        <p><span onmouseenter={()=>hover("סך ההצבעות בעד")} onmouseleave={()=>hover("0")}  style="color:#7EE081;" >{noofusersOk}-בעד</span> <span onmouseenter={()=>hover("לא הצביעו")} onmouseleave={()=>hover("0")}  style="color:#0000cc;" >{noofusersWaiting}-טרם </span><span onmouseenter={()=>hover("כמות ההצבעות על גרסאות קודמות")} onmouseleave={()=>hover("0")}  style="color:#80037e;" >{noofusersNo}-על גרסה קודמת</span></p>
      </div>
       </div>
       {#if low == false}
 {#if already === false && allr === false}
    ...
    {:else if already === true && mypos === true && noofusersNo > 0 && allr === false}
            <button onmouseenter={()=>hover("משא ומתן")} onmouseleave={()=>hover("0")}  
                onclick={() => nego("alr")} 
                class = "btna bg-gradient-to-br from-gra via-grb via-gr-c via-grd to-gre hover:from-barbi hover:to-mpink hover:text-gold text-barbi hover:scale-110" 
                name="negotiate" 
