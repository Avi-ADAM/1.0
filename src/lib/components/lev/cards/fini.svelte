  ...
  <div  class="{isScrolable ? "bg-white" : "bg-gray-200"} transition-all-300  rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
    <div  class="mb-8">
         <p style="line-height: 1;" class="text-sm text-gray-600 flex items-center">
            ...
            <span onmouseenter={()=>hover(`${tri.common.valph[$lang]}`)} onmouseleave={()=>hover("0")} > {valph} {tr.common.perhour[$lang]} </span> * <span onmouseenter={()=>hover(`${tri.commom.noofhours[$lang]}`)} onmouseleave={()=>hover("0")}  > {nhours.toLocaleString('en-US', {maximumFractionDigits:2})} {tr.common.hours[$lang]} </span> = <span onmouseenter={()=>hover(`${tr.common.total[$lang]}`)} onmouseleave={()=>hover("0")}>{(nhours * valph).toLocaleString('en-US', {maximumFractionDigits:2})} </span>
      </p>
    ...
    {#if hearotMeyuchadot}
     <p onmouseenter={()=>hover(`${tr.common.notes[$lang]}`)} onmouseleave={()=>hover("0")} class="text-grey-700 max-h-16 cd text-sm d">{hearotMeyuchadot !== undefined && hearotMeyuchadot !== null && hearotMeyuchadot !== "undefined" ? hearotMeyuchadot : ""}</p>
     {/if} 
     <div class="flex items-center border border-gold flex-col" >
      {#if why !== undefined && why !== null && why !== "undefined"}
      {#if why.startsWith("http")}
          <a onmouseenter={()=>hover(`${tr.common.finalwords[$lang]}`)} onmouseleave={()=>hover("0")} class="text-grey-700 max-h-16 max-w-full cd text-sm d hover:text-barbi" target=”_blank” href={why}>link</a>
      {:else}
          <p onmouseenter={()=>hover(`${tr.common.finalwords[$lang]}`)} onmouseleave={()=>hover("0")} class="text-grey-700 max-h-16 cd text-sm d">{why}</p>
      {/if}
      {/if}
          {#if whatt}
           <a onmouseenter={()=>hover(`${tr.finiapp.file[$lang]}`)} target=”_blank” onmouseleave={()=>hover("0")} href="{whatt}" class="text-barbi hover:text-gold">{tr.finiapp.file[$lang]}</a>
          {/if}
        </div>
        </div>
     <div class="flex items-center">
      ...
      <div class="text-sm">
        ...
        <p class="vo ef"><span onmouseenter={()=>hover(`${tr.vots.totalin[$lang]}`)} onmouseleave={()=>hover("0")}  style="color:#7EE081;" >{noofusersOk}-{bead[$lang]}</span> <span onmouseenter={()=>hover(`${tr.vots.notyet[$lang]}`)} onmouseleave={()=>hover("0")}  style="color:#0000cc;" >{noofusersWaiting}-{notyet[$lang]} </span><span onmouseenter={()=>hover(`${tr.vots.totalno[$lang]}`)} onmouseleave={()=>hover("0")}  style="color:#80037e;" >{noofusersNo}-{neged[$lang]}</span></p>
      </div>
    </div>
       </div>
