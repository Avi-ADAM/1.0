 ...
 <!-- <div class="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden bg-gold" style:background-image={`url('${src2}')`} title="">
      ...
      <div class="relative flex items-center space-x-1">
         ...
         <div class="flex flex-col leading-tight">
            <div class="sm:text-sm text-md mt-1 flex items-center">
               <span class="text-barbi text-center mr-3 sm:text-xl text-sm"> הצבעה על קבלת משאב בהצלחה  </span>
            </div>
  ...
  <div  class=" bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
    <div  class="mb-8">
      ...
      {#if kindOf === "perUnit"}
       <p ><span on:mouseenter={()=>hover(" שווי ליחידה")} on:mouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >{agprice.toLocaleString('en-US', {maximumFractionDigits:2})}ליחידה</span> * <span on:mouseenter={()=>hover("כמות")} on:mouseleave={()=>hover("0")} style="color:var(--barbi-pink);" >{hm == 1 ?  `יחידה אחת ` : `${hm} יחידות`}</span> = <span on:mouseenter={()=>hover("סך הכל")} on:mouseleave={()=>hover("0")} >{(agprice * hm).toLocaleString('en-US', {maximumFractionDigits:2}) }  סך הכל</span> </p>
   {:else if kindOf === "total" || kindOf === "rent"}
       <p ><span on:mouseenter={()=>hover("שווי מוצע")} on:mouseleave={()=>hover("0")} style="color:var(--barbi-pink)" >{agprice.toLocaleString('en-US', {maximumFractionDigits:2})}</span></p>
          {:else if kindOf === "monthly"}
       <p ><span on:mouseenter={()=>hover("שווי לחודש")} on:mouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >{agprice.toLocaleString('en-US', {maximumFractionDigits:2})} לחודש</span> * <span on:mouseenter={()=>hover("כמות חודשים")} on:mouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >{monts == 1 ?  `חודש אחד` : `${monts} חודשים`}</span> = <span on:mouseenter={()=>hover("סך הכל")} on:mouseleave={()=>hover("0")} >בסך הכל {(agprice * monts).toLocaleString('en-US', {maximumFractionDigits:2}) }</span> </p>
          {:else if kindOf === "yearly"}
       <p ><span on:mouseenter={()=>hover("שווי לשנה")} on:mouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >{agprice.toLocaleString('en-US', {maximumFractionDigits:2})} לשנה</span> * <span on:mouseenter={()=>hover("מספר שנים")} on:mouseleave={()=>hover("0")}  style="color:var(--barbi-pink)" >{yers == 1 ?  `שנה אחת` : `${yers} שנים`}</span> = <span on:mouseenter={()=>hover("סך הכל")} on:mouseleave={()=>hover("0")} > בסך הכל { (agprice * yers).toLocaleString('en-US', {maximumFractionDigits:2}) }</span> </p>
{/if}
         </div>
