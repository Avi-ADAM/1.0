...
function toggleShow (){
 ...
 $: w = 0;
 let u = {"he":`אישור ${kind == "send" ? "העברת" : "קבלת"} כספים על פי החלוקה שנקבעה`,"en":`apruval of ${kind == "send" ? "sending": "reciving"} money acording to the appruved spliting`}
...
<div class="text-center">
  <img
    ...
    alt="Avatar"
  />
  <h5 class="text-s font-medium leading-tight mb-2">{sendname}</h5>
  <!----<p class="text-gray-500">Web designer</p>-->
</div>
        </div>
...
<!--
 ...
 >
   ...
   {#if low == false}
     {#if already === false}
   <button on:mouseenter={()=>hover(apru[$lang])} 
   ...
   <button on:mouseenter={()=>hover(mes[$lang])} 
            on:mouseleave={()=>hover("0")} on:click= {react} style="margin: 0;" class = "btn b" name="negotiate" ><Chaticon/></button>
   <button on:mouseenter={()=>hover(noo[$lang])} 
