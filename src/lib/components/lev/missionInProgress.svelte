...
<button onmouseenter={()=>hover("לחיצה לעצירת הטיימר ושמירת הזמן שבוצע")} onmouseleave={()=>hover("0")} class="  bg-gradient-to-br text-gold hover:from-graa hover:to-grab hover:text-gold   p-0 rounded-full from-lturk to-barbi gb" onclick={save}> הוספה</button>
{/if}
    {#if already === false}
...
<style>
  @import url(https://fonts.googleapis.com/css2?family=Abel%3Aital%2Cwght%400%2C400&amp;display=swap);
   #hours { stroke: #00ffff; }
   #hhand { fill: #00ffff; stroke: purple; }
   #minutes { stroke: lime; }
   ...
   text { fill: #00ffff; stroke: purple; stroke-width: 0.75px; }
    .swiper-slidec {
  display: flex;
  align-items: center;
