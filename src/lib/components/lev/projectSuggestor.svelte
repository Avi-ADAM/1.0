...
use:clickOutside on:click_outside={toggleShow}>
   ...
   {#if deadLine} <h5 class="lt">{deadLine}</h5>{/if}
    ...
    <button on:click={agree(oid)} class="btn a" name="requestToJoin" title="אני רוצה"><i class="far fa-check-circle"></i></button>
   <!--<button on:click={nego(oid)} name="negotiate" class="btn" title="משא ומתן"><i class="far fa-comments"></i></button>
   -->  <button   on:click={decline(oid)} class="btn b" name="decline" title="לא מתאים לי"><i class="far fa-times-circle"></i></button>
