...
function project () {
 ...
 onMount(function(){
 if ($lang != "en" ){
              for (var x = 0; x < skills.data.length; x++){
                if (skills.data[x].attributes.localizations.data.length > 0){
                skills.data[x].attributes.skillName = skills.data[x].attributes.localizations.data[0].attributes.skillName
                }
              }
              for (var u = 0; u < role.data.length; u++){
                if (role.data[u].attributes.localizations.data.length > 0){
                role.data[u].attributes.roleDescription = role.data[u].attributes.localizations.data[0].attributes.roleDescription
                }
              }
              console.log(workways)
              for (var r = 0; r < workways?.data.length; r++){
                if (workways.data[r].attributes.localizations.data.length > 0){
                workways.data[r].attributes.workWayName = workways.data[r].attributes.localizations.data[0].attributes.workWayName
                }
              }
            }
            role = role
