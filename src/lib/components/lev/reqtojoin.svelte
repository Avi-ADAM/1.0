...
{:else}
        <div class="normSmlHover"
            ...
            {#if !already}
            <button1 on:click={agree}  class = "btn a" name="requestToJoin"><i class="far fa-check-circle"></i></button1>
          <!-- <button3 on:click= {ask} style="margin: 0;" class = "btn" name="negotiate"><i class="far fa-comments"></i></button3>--> 
            <button2 on:click={decline}  class = "btn b"name="decline"><i class="far fa-times-circle"></i></button2>
        {/if}
        </div>
        {/if}
        </div>
