...
- **Resource cards (huca)** still render from the legacy `sps →
  open_mashaabims` extraction in query 83; resource match-suggestion rows and
  emails are already written. Migrating the huca display to the collection
  (and then slimming the `sps` block of query 83) is the next step.
- The legacy `timegrama/pend.svelte` silence-approval endpoint also creates
  open missions and is not hooked yet.
- Location (`new.location` on open-mission) can now be added as a filter in
  one place — `engine.ts` — instead of client code.
- `extractSuggestions` stays as a fallback shell (ask-derived cards only);
  remove once 209 is proven in production.
