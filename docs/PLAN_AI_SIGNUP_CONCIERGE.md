...
| רקמה חדשה עם מוצרים בלבד, שרק אני חבר בה | **קליק אחד במסך האישור** — גם כאן (לא יצירה ישירה מהצ'אט). |
| שותפים במסלול `partnership` | **הזרימה הרגילה:** מאשרים במסך, המערכת שולחת מיילים, ולשותף יש כרטיס לב כאילו המשימה נוצרה בלי קשר לייבוא (§4.5 שלב 5). טיוטה משותפת — שלב מתקדם, לא עכשיו. |
| מוצר שהוסתר מהספרייה (`hideFromDiscovery`) | גם הקונסיירז' לא מציע אותו — **תוקן** ב-qid 203. |
...
- השרת (`materializeRikmaBlueprint`, ב-VPS) מריץ **את אותן actions** שהטפסים מריצים —
  ...
  5. שותפים ← הזמנה **בזרימה הרגילה**: המשימות/המשאבים שהשותף מביא נוצרים
     פתוחים (שלב 3–4), ולשותף נוצרת הצעת התאמה ממוקדת (`match-suggestion`,
     `source:'invited'`) + מייל "יש לך הצעה חדשה" — כלומר כרטיס לב (`sugestmi`/
     `sugestma`) בדיוק כמו התאמה רגילה. משם הוא מגיש מועמדות, והרקמה מאשרת
     בזרימת ה-ask הקיימת, וכך הוא נהיה חבר. שותף שאין לו חשבון לפי המייל ⇒
     מייל הזמנה עם קישור ל-`/project/[id]/join` (דף ההצטרפות הקיים);
...
7. **הוחלט (2026-09-24):** קליק אחד גם לרקמה חדשה של חבר יחיד; שותפים בזרימה
   הרגילה (מיילים + כרטיס לב), טיוטה משותפת בהמשך; `hideFromDiscovery` מכובד
   בקונסיירז' (תוקן). ראו §0.1.
