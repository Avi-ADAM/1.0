...
>
> **אבל השער הציבורי פתח שתי שאלות נוספות** — לא "מי מדבר עם Strapi" אלא
> "מי הלקוח שמדבר עם השער", ואז "על איזו שורה". הזהות נגזרה מ-cookie שהלקוח
> כותב (**§14.1**, נסגר 2026-09-02), ו-114 מוטציות פנו לשורה קיימת לפי id
> מהלקוח בלי בדיקת בעלות (**§14.2**, המנגנון נבנה 2026-09-03: ליבת הכסף
> נאכפת, השאר ב-shadow עם מבחן כיסוי ב-CI). §14.3 ו-§14.4 סגורים.
...
#### ה-`.env` המקומי — שני מצבים, וזה שנבחר (2026-09-02)

**מצב A — פיתוח רגיל (ברירת המחדל, מוגדר עכשיו):**

```
VITE_API_BASE=
SSR_API_BASE=
STRAPI_URL=https://tovmeod.1lev1.com
STRAPI_GATE_KEY=…            # חייב להיות זהה לשרת
```

הכל רץ מקומית: ה-`/api/*` שלך, qids ו-actions חדשים עובדים מיד, ו-`hooks.server.js`
חותם `x-strapi-gate` כך ש-Strapi הנעול פותח לך. אומת ב-2026-09-02 מול השער
**הסגור**: עם המפתח 200, בלעדיו 403. אחרי שינוי ב-`.env` צריך **להפעיל מחדש את
שרת ה-dev** — Vite קורא אותו רק בעלייה.

**מצב B — אימות בצורת פרודקשן** (למלא את שני ה-bases ולהמית את `STRAPI_URL`,
כמתואר למעלה). רק לבדיקת ניתוב לפני הדלקה ב-Vercel — הוא מריץ UI מקומי מול קוד
שרת **פרוס**, אז qid חדש שכתבת מקומית לא קיים שם.
...
- [x] ~~401 בודד בטעינת `/lev`~~ — **נסגר 2026-09-02: לא באג.** הלוג
      (`GET /api/permissions`, `status:401`, `uid:null`, `ua: …Claude/1.40609.1…`)
      מראה שזו הייתה בדיקה אנונימית שהרצתי בעצמי על `/love` **לפני** ההתחברות;
      קונסול הדפדפן מצטבר בין ניווטים, ולכן היא הופיעה שוב ברשימה של `/lev`.
      אין 401 שמקורו באפליקציה.
...
## 14. הזהות מאחורי השער והשורה שמאחוריה (2026-09-02 → 2026-09-03)

§10–§13 סגרו את השאלה **"מי מדבר עם Strapi"**: רק הפרוקסי. הן לא נגעו בשאלה
הבאה, שנפתחה בדיוק ברגע שהשער הפך לציבורי ב-`api.1lev1.com`: **"מי הלקוח
שמדבר עם הפרוקסי"**.

### 14.1 הזהות נלקחה מ-cookie שהלקוח כותב — **נסגר ✅ (2026-09-02)**

`/api/auth` כותב `jwt` כ-`httpOnly:true`, אבל את `id` / `un` כ-`httpOnly:false`
כדי שה-UI יקרא אותם. אף אחד לא חותם עליהם. משם הם הגיעו לכל מקום:

| מקום | מה נגזר מה-cookie | המשמעות |
|---|---|---|
| `api/action/+server.ts` | `context.userId` | **הגדול מכולם.** כל כלל `self` / `projectMember` ב-Action System משווה לערך הזה. |
| `api/send/+server.js` | `idL`, `un`, ה-principal | ההחלפה `key === 'idL' ? idL : …` נראתה כמו הקשחה, אבל דרסה ערך מהלקוח בערך אחר מהלקוח. השומרים `170getMyCoMembers` / `307myIncomeHistory` השוו `uid` מהלקוח מול `callerId` מהלקוח. |
| `hooks.server.js` | `locals.uid` / `locals.un` | ~45 קריאות ב-SSR. |
| `api/api-keys` | בעל המפתח | הנפקת מפתח API בשם משתמש אחר. |
| `api/consent/*`, `approveHaluka`, `onboard/save`, `nutiUser`, `nutifyPm` | המשתמש הפועל | |

`httpOnly:true` לא היה מתקן את זה: השער ציבורי, ו-
`curl -H 'Cookie: jwt=<אמיתי>; id=<של מישהו אחר>'` לא עובר בדפדפן בכלל.

**מה נעשה.** מודול אחד — `src/lib/server/identity.js` — הוא מעכשיו המקום היחיד
שבו השרת לומד מי הקורא, והוא גוזר את זה **מהטוקן החתום בלבד**. שני מאמתים:

1. **אימות HS256 מקומי** מול `JWT_SECRET` (אותו סוד של Strapi). זו הדרך הכי
   מאובטחת וגם הכי זולה — בלי רשת בכלל — ולכן היא הנכונה ל-`api.1lev1.com`,
   שיושב על אותה מכונה עם Strapi. טוקן מזויף נדחה **לפני** שהוא נוגע במשהו.
2. **שאילת Strapi** (`me { id username }` עם הטוקן של הקורא). עובדת בלי סוד
   משותף, ולכן פרונט שאינו co-located (Vercel) מוגן גם הוא. זה גם המקור היחיד
   ל-`username` — payload של טוקן Strapi הוא `{id, iat, exp}` בלבד, ו-`username`
   הוא מזהה אבטחה לפחות במקום אחד (voter-id בהצבעות consensus).

התוצאה נשמרת ב-cache לפי טוקן ל-5 דקות (ולעולם לא מעבר ל-`exp` שלו), כך שהמחיר
הוא בערך פנייה אחת ל-Strapi למשתמש מחובר לחמש דקות. כששני המאמתים עונים, הם
חייבים להסכים. טוקן שלא הצלחנו לאמת הוא **לא** session: ה-principal יורד ל-
`anonymous` וה-authz דוחה (fail closed).

`hooks.server.js` פותר פעם אחת לבקשה ומפרסם ב-`locals.uid` / `locals.un`; כל
נתיב אחר קורא משם. פרונט שמנתב `/api/*` הלאה (Vercel — `SSR_API_BASE` מוגדר)
שואל דרך אותה דלת קדמית: **`/api/whoami`** על ה-VPS, שקורא רק את ה-cookie ולא
מכבד את ה-internal secret כהעלאת הרשאה.

- [x] `src/lib/server/identity.js` + `src/lib/server/identity.test.ts` (17 בדיקות)
- [x] `src/routes/api/whoami/+server.js`
- [x] `hooks.server.js`, `api/send`, `api/action`, `api/permissions`,
      `api/api-keys`, `api/consent/*`, `approveHaluka`, `onboard/save`,
      `nutiUser`, `nutifyPm`, `action-service-example`
- [x] `resolveCookiePrincipal` → `resolveSessionPrincipal(cookies, identity)`
- [ ] **פריסה: `JWT_SECRET` ב-`.env` של `sveltekit-api`** — אותו ערך כמו ב-Strapi.
      בלעדיו המערכת עובדת ובטוחה, אבל דרך מסלול 2 בלבד (פנייה ל-Strapi).
      עם הסוד: אימות מקומי, בלי רשת, ועמידות לנפילת Strapi. החלפה = restart,
      לא build (אותו נוהל כמו §13.3).

> **קו אדום להמשך:** `cookies.get('id')` / `cookies.get('un')` בקוד שרת הם באג.
> המקור הוא `locals.uid` / `locals.un`. (`signup/+page.server.js` הוא היוצא מן
> הכלל היחיד — שם `un` הוא ערך ברירת-מחדל לטופס הרשמה, לפני שיש session בכלל.)

### 14.2 IDOR במוטציות של `/api/send` — **המנגנון נבנה 2026-09-03; ליבת הכסף נאכפת, השאר בצל**

487 qids, מהם 235 מוטציות, מהן 201 מותרות ל-principal מסוג `user`, ומתוכן
**114 פונות לשורה קיימת לפי id מהלקוח** — צורת ה-IDOR. מולן היו 4 שומרים
ב-`guards.js`, ושניים מהם על קריאות.

**ממצא שמשנה את גודל החשיפה:** כל ה-qids הכספיות שהאודיט מנה נקראות בפועל דרך
`strapi.execute` (actions ומודולי שרת) שפונה ל-Strapi **ישירות**, לא דרך
`/api/send`. כלומר הפרצה לא הייתה בזרימות עצמן אלא בכך ש-`/api/send` הרשה
ל-`user` להריץ אותן בעצמו, בלי שאף אחד שואל על איזו שורה. זה בדיוק מה שנסגר.

**מה נבנה.**

| קובץ | תפקיד |
|---|---|
| `qidTargets.js` | גוזר מה-GraphQL של ה-qid עצמו *לאיזו שורה הוא כותב* ו*אילו מערכי component הוא מחליף בשלמותם*. אין רשימה ידנית שיכולה להתיישן. |
| `ownership.js` | המניפסט + הבודק. מפתח לפי **ישות**, לא לפי qid, כך ש-qid חדש על ישות מוכרת מכוסה ביום שנכתב. |
| `voteIntegrity.js` | ה-mass assignment של `$vots`. |
| `guardMode.js` | `SEND_GUARD_MODE` — `enforce` (ברירת מחדל) / `log` / `off` (מסורב מחוץ ל-dev). |
| `ownership.test.ts`, `voteIntegrity.test.ts` | 37 בדיקות, כולל **מבחן הכיסוי**: ישות שמוטציה פונה אליה ואין לה כלל — ה-CI נכשל. זה מה שהופך את השומרים ל-deny-by-default כמו ה-qids. |

**השאלה שהשכבה שואלת** היא לא "מותר לך לעשות את זה" אלא החלשה והבטוחה בהרבה
**"השורה הזאת עניינך בכלל"**: הקורא חייב להיות רשום על השורה עצמה, או חבר ברקמה
שהשורה שייכת לה. כל זרימה לגיטימית עומדת בזה; IDOR לא. השאלות הצרות — תורך? אתה
המחזיק? ההצבעה נסגרה? — נשארות היכן שהן כבר חיות. שאילתה אחת לכל מוטציה מביאה גם
את יחסי המשתמש של השורה וגם את חברי הרקמה.

**מה נאכף עכשיו** (`enforce: true`): `haluka`, `tosplit`, `sale`, `sheirut`,
`sheirutpend`, `siteShareContribution`, `project`, `usersPermissionsUser`,
`matchSuggestion` — 34 qids, כולל ארבע הדוגמאות מהאודיט. השאר (~30 ישויות)
ב-shadow: מלוגג `[ownership-shadow]` ולא חוסם.

**שסתום ביטחון:** שורה שאף יחס בעלות שלה לא מאוכלס לא נחסמת — היא מלוגגת
בקול (`[ownership] … has no owner relations set — allowing`). פער דאטה הוא לא
פורץ, וחסימה שם הייתה שוברת זרימה לגיטימית בלי לסגור כלום.

- [x] המנגנון, המניפסט (40 ישויות), מבחן הכיסוי ב-CI
- [x] אכיפה על ליבת הכסף
- [x] `$vots` — `voteIntegrity.js` דורש שכל **מספר** הרשומות של כל משתמש אחר
      יישמר. לא נבדק תוכן הרשומה (זרימת אישור מנרמלת `zman`/`order`), לא כמה
      הקורא מוסיף לעצמו. נאכף על `tosplit`, בצל על השאר.
- [ ] **הבדיקה הידנית שנשארה, 30 שניות בפאנל האדמין:** האם ל-`Authenticated`
      יש `update` על `haluka` / `tosplit` / `sheirut`? זה קובע אם מה שנסגר היה
      חי או תיאורטי. (בריפו `1.0b`: 48 קבצי `lifecycle.js` ריקים ושתי policies
      בסך הכל — אין אכיפת בעלות בצד Strapi.)
- [ ] **הרחבת האכיפה לשאר הישויות** אחרי שהלוגים של `[ownership-shadow]`
      משעממים. הסיכון היחיד הידוע הוא זרימות מועמד: מי שעדיין לא חבר ברקמה
      ומנהל מו״מ (`negoUpdateOpenMission`, `131bArchiveAskm`, `126updateSpDeclined`).
      הכללים כבר כוללים את רשימות המשתמשים שבהן מועמד מופיע — הלוגים יגידו אם זה מספיק.

### 14.3 שלושה קטנים — **נסגרו ✅ (2026-09-03)**

- [x] **בניית GraphQL בשרשור מחרוזות.** מסלול ההצבעה ב-`api/send/+server.js`
      ו-`UpdateClause` ב-`guards.js` עברו ל-variables. במסלול ההצבעה זה שתי
      שאילתות ליטרליות שנבחרות לפי ה-qid, ולא שם ישות מוזרק — כדי שזה יישאר
      נכון גם אם הגזירה תשתנה.
- [x] **שגיאות Strapi חוזרות ללקוח.** קורא שהוכיח שהוא השרת (`isInternalRequest`)
      ממשיך לקבל את מערך ה-errors כמו שהוא — actions, jobs ו-SSR loads מפרסרים
      אותו. דפדפן מקבל `DOWNSTREAM_ERROR` + `reqId`, והפירוט נשאר בלוג עם אותו id.
- [x] **`AUTHZ_MODE` דינמי.** `off` מחוץ ל-dev זורק במקום להגיש בלי שכבה;
      `log` נשאר (הוא ה-rollback המתועד) אבל מכריז על עצמו פעם אחת בלוג.
      `SEND_GUARD_MODE` נבנה באותה צורה בדיוק.

### 14.4 שני תיקונים בדרך (2026-09-03)

מבחן הכיסוי של `qidsAccess` היה אדום מלפני העבודה הזאת, ובדיקה אדומה מרוקנת
מתוכן את אותה משמעת שזה עתה הוספנו:

- [x] **`1chatsendGuest` לא היה מסווג.** נקרא רק מ-`/api/guest/message` עם
      טוקן האדמין, ואחרי בדיקה שהפורום באמת שייך לפגישה של האורח. סווג
      `serviceAdmin` בלבד — דפדפן שהיה מגיע אליו ישירות יכול היה לפרסם בכל
      פורום תחת כל שם אורח.
- [x] **`59GetMeetingDetails` בלי `serviceMeetings`.** ה-principal של אפליקציית
      הפגישות היה קיים בקוד בלי ולו qid אחד מורשה — כלומר הפרוקסי שלה מת, לא
      מוקשח. הוענק, והבדיקה שוב היא ה-tripwire.
