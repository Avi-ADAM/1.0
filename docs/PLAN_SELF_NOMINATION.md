# תכנית: הצעה עצמית לרקמה (Self-Nomination)

אדם שמתחבר לכיוון של רקמה מציע את עצמו מהדף הציבורי `/project/[id]`:
...
מבשילה, הרקמה רשאית **לבטל את המשימה/המשאב כליל**, כי הם מעולם לא היו שלה.

נוצר: 2026-07-07 · עודכן: 2026-07-17 · סטטוס: **מומש חלקית** (ר' ★ למטה)

הבהרה חשובה: ההצעה העצמית רלוונטית ל**כל** סוג רקמה — מסחרית, התנדבותית
...
("אני אעשה…" משימה / "אני אביא…" משאב), ערכי הרקמה כהקשר, הסבר
...
- S4b (2026-09-05): לדף ה-join נוסף לוח **"מה הרקמה מחפשת עכשיו"** —
  המשימות הפתוחות *וגם* המשאבים הפתוחים של הרקמה (qid
  `311projectOpenBoardPublic`, אותם מסננים ציבוריים של 49), כל פריט מקשר
  ל-`/availableMission/[id]` / `/availiableResorce/[id]` שבהם כפתורי
  המועמדות הקיימים. הטופס נשאר מתחתיו: קודם לקחת משהו פתוח, ורק אז להציע
  משהו חדש. הלוח הוא best-effort — כשל בשאילתה לא מונע את הטופס.
- S4c (2026-09-05): **חבר רקמה רואה עכשיו את כל העמוד**, לא רק את כרטיס
  "את/ה כבר חלק": הכרטיס (עם ההפניה למוח) יושב מעל הלוח, ובמקום טופס
  ההצעה העצמית — שהוא חסר משמעות לחבר — מוצגות אותן שתי הבחירות
  כקישורים ל-`/moach/[pid]/create?action=createmission` /
  `?action=createresource`, שפותחים שם מיד את הטופס המתאים. הצרכן של
  `action=createresource` נוסף בדף ה-create (תאום ל-`createmission`
  הקיים) ומקבל גם `name`/`descrip`/`price`/`quantity`/`kindOf`.
- qids: 214–218 (יצירת OpenMission/OpenMashaabim/Sp להצעה עצמית +
  שאילתות ההקשר לביטול), 311 (הלוח הציבורי).
...
- **צד המועמד (משאבים)** ✅: `appendMyPendingAskmEntries` ב-extractors
  ממזג לכרטיסי ה-huca כל Askm ממתין של המשתמש שלא הגיע מ-match-record
  או מהילוך ה-sps — כולל משאב בהצעה עצמית — כך שלמועמד יש תמיד כרטיס
  בלב על ההצעה שלו: צ'אט (addAskmChatEntry), צפייה בנגדית של הרקמה
  ואשרור/נגדית (B2), ו"🌱 משיכת ההצעה" על הצעה עצמית. המיזוג רץ בשני
  הנתיבים (extractResourceSuggestions הישן + builder ה-match-records),
  וכרטיס `sugestma` קיבל מצב "ממתין" עם כפתורי צ'אט/משיכה (קודם לא
  היו לו פעולות בכלל במצב הזה — התיקון משרת כל הצעת משאב חיצונית).
- הערת פער ידוע: כרטיסי ה-UI החלופיים (`coinui`/`newcoinui`) לא קיבלו
  את הבאדג'.

**נותר**: S6 בדיקות unit לפעולות, S7 גילוי (hub), ובאדג' ב-UI החלופי.
...
|---|---|---|
| מי יוצר את ה-OpenMission/OpenMashaabim | הרקמה (pendm/pmash) או קונסיירז' | **המועמד** |
| הבסיס (baseline) | תנאי הרקמה | **תנאי המועמד** |
...
- **שער דו-צדדי** — `src/lib/server/nego/negoGate.ts` (`computeNegoGate`).
  אין סבב ⇒ מצב-בסיס: נדרשת הצבעת-בעד של חבר-רקמה אחד + העדר `false`.
...
- **timegrama נדחה** — נוצר רק בהצבעת-בעד ראשונה של חבר
  ...
  שתיקה היא הסכמה בקצב הרקמה (`restime`) — רק אחרי שחבר נענה.
- **הצעה נגדית של הרקמה** — `counterOnAsk`/`counterOnAskm` (B2), והמועמד
...
- **כרטיסי לב** — ה-Ask שייך לפרויקט ⇒ נטען ב-`83levMainUserQuery`
  (project-scoped) ומופיע לחברי הרקמה ב-`reqtom`/`reqtojoin`; ולמועמד
...
- על **עצם הקשר** מותר לרקמה לומר "לא רלוונטי לנו כרגע" — ואז המשימה
  שהמועמד יצר לא נשארת זבל פתוח אצל הרקמה, אלא מתארכבת כולה.
...
1. אימות: המבקש **אינו** חבר רקמה (`128getProjectMembersAndRestime`);
...
6. נוטיפיקציה לחברי הרקמה (`specificUsers`, socket+push):
   "מישהו מציע את עצמו לרקמה שלכם".
...
ההבדל היחיד במכונה. חבר רקמה מפעיל:

1. אימות: המפעיל חבר רקמה; ה-OpenMission/OpenMashaabim הוא
...
4. נוטיפיקציה מכבדת למועמד, עם הזמנה להמשיך לעקוב אחרי הרקמה.
...
- CTA חדש בולט אחרי סקשן הערכים: **"מתחבר/ת לכיוון? הציעו את עצמכם לרקמה"**.
- לא-רשומים: ה-CTA מוביל להרשמה (עם `redirect` חזרה לדף), כמו כרטיס ההזמנה
  הקיים.
- חבר רקמה: ה-CTA מוסתר (יש לו את המוח).
- לחיצה פותחת מודאל דו-שלבי:
  1. **מה אני מציע** — משימה ("אני אעשה…") או משאב ("אני אביא…").
  2. **התנאים שלי** — טופס במתכונת `<NegoM>`/`<Nego>` הקיימים
     (name, descrip, שעות/מחיר או שדות משאב, מיומנויות). ערכי הרקמה
     מוצגים לצד הטופס כהקשר.
- שליחה → `nominateSelfMission`/`nominateSelfResource` → אישור
  "ההצעה נשלחה — לרקמה יש [restime] להגיב מרגע שחבר ייענה".

### 4.2 כרטיסי הלב

- **לחברי הרקמה** (`reqtom`/`reqtojoin`): הכרטיס הקיים + באדג' "הצעה עצמית
...
## 5. הערת אגב שהופכת לשלב: גילוי רקמות (Discovery)

ההצעה העצמית שווה משהו רק אם אנשים מגיעים לדפי רקמות. היום אין שום עמוד
...
1. **P1 — "רקמות שמתחברות אליך" בהאב** (`(reg)/hub`): סקשן של עד 5 רקמות
   ...
   נטען idle-time.
2. **P2 — עמוד גילוי `/projects`** (ציבורי, `(regandnon)`): רשימת רקמות עם
   פילטור לפי ערכים (Vallue) ותחומים, חיפוש טקסטואלי. זה גם דף SEO-כניסה.
3. **P3 — הצעה בפרופיל ובאונבורדינג**: בסוף מסלול onboarding (יש
   `onboarding_track`) ובדף `/user/[id]` של עצמך — "רקמות בכיוון שלך".
4. **P4 — התאמה יזומה בסגנון קונסיירז'**: ג'וב תקופתי (במתכונת
   `RatsonMatchJob`) שמזהה חיתוך גבוה משתמש↔רקמה ושולח נוטיפיקציה. רק אחרי
   שיש דאטה מ-P1–P3.
...
## 6. חיבור לתכנית הרקמה ההתנדבותית

ר' `docs/PLAN_VOLUNTEER_RIKMA.md`. נקודת החיבור: כשהרקמה היא
...
- [ ] **S7 גילוי P1**: סקשן רקמות מותאמות בהאב.
