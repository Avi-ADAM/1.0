...
### `mastra_threads` / `mastra_messages` — היו 0 עד 2026-08-05

בבדיקה הראשונה הטבלאות האלה היו ריקות, וזה נראה כמו באג באחסון. זה לא היה:
`storage` ברמת ה-`Mastra` מזין telemetry/workflow-snapshots, אבל **זיכרון שיחה
נכתב רק ע"י instance של `Memory` שמחובר ל-agent** — ואף agent לא הגדיר
`memory:`.

מאז חוברו סוכני הצ'אט ל-`Memory` (threads, messages ו-`mastra_resources`
לפרופיל המובנה). ראו [`PLAN_CHAT_MEMORY.md`](PLAN_CHAT_MEMORY.md). המשמעות
לאחסון: הטבלאות האלה כבר לא ריקות אלא גדלות עם כל תור של כל משתמש — מה שהופך
את סעיפים 5 (ניקוי) ו-6 (גיבוי) למשהו שצריך לתזמן, לא לרשום.

## 5. שמירה על גודל ה-DB

**אין כרגע שום מנגנון ניקוי.** traces, evals והודעות יגדלו בלי גבול — ומאז
חיבור הזיכרון (`PLAN_CHAT_MEMORY.md`) הודעות הן הרוב הגדל.
