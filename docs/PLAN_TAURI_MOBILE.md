...
## ⚠️ החלטה (2026-08-19): מעטפת לאתר החי עכשיו, SPA כמסלול המשך

התכנית שלמטה (שלבים 1–2) היא **מסלול ה-SPA**. הוא לא בוטל, אבל הוא **לא**
המסלול שהאפליקציה רצה עליו היום. אחרי שראינו מה עבד בפרויקט האח
`106_ezrachi` (‏`docs/mobile-app-plan.md` שם), ה-build הפעיל הוא **מעטפת שטוענת
את האתר החי**:

- `app.windows[0].url = https://www.1lev1.com/?app=1` · `frontendDist: ./shell`
  (מסך offline מקומי זעיר).
- **האימות פשוט עובד**: ה-origin הוא האתר עצמו, ולכן ה-cookie ‏(`jwt`, HttpOnly,
  same-origin) נשלח כרגיל. **כל שלב 2 של התכנית הזאת — CORS, Bearer,
  `getAuthToken` — לא נדרש למעטפת**, וגם לא שלב 1 (‏`/api/auth/me`, guards).
- ה-identifier הוא `world.onelev1.app`. הישן, `world.1lev1.app` שב-Capacitor,
  **אינו שם חבילה חוקי** (מקטע לא יכול להתחיל בספרה) ולכן הפרויקט ההוא מעולם
  לא נבנה. אין רישום ב-Play שנעול עליו.
- הפרטים המלאים — מסך פתיחה, אייקון, הרשאות IPC לתוכן מרוחק — ב-`src-tauri/README.md`.

**מה מסלול ה-SPA עדיין קונה, וזו הסיבה שהוא נשאר בתכנית:**

| | מעטפת (היום) | SPA ארוז |
|---|---|---|
| טעינה ראשונה | סבב רשת (~2.4 שנ' נמדד ב-106) | מיידית, מנכסים מקומיים |
| אופליין / local-first | האפליקציה לא עולה בלי רשת | שכבת ה-Space מ-IndexedDB עובדת |
| פלאגינים נייטיביים | דורשים `remote.urls` — וב-אנדרואיד כל iframe בעמוד מגיע לאותן הרשאות | IPC מקוד מקומי, ברירת מחדל בטוחה |
| גרסאות | פריסה שבורה של האתר מפילה את המשתמשים בשטח; תיקון מגיע בלי עדכון בחנות | לאפליקציה גרסה משלה |
| Google Play | חשוף למדיניות "פונקציונליות מינימלית" | עומד בפני עצמו |

התשתית של מסלול ה-SPA (`scripts/build-mobile.mjs`, `$lib/platform/index.js`,
ענף `ADAPTER=mobile` ב-`svelte.config.js`) **נשארת במקומה**, ו-`src-tauri/tauri.spa.conf.json`
מחזיק את ההפרש בקונפיגורציה. לחזרה למסלול הזה צריך להשלים את 1.1–1.3 ואת שלב 2.
