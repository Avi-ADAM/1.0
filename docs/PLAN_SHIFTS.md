...
מכסות: 5 מקומות / 3 מועמדים ⇒ [2, 2, 1] בשיטת השארית הגדולה (לפי חוב היסטורי;
נניח [רון 2, דנה 2, יואב 1]).

| סדר | המקום הנדיר | הזוכה | למה |
|---|---|---|---|
| 1 | ה' (מועמד אחד) | רון | יחיד |
| 2 | א' (שניים) | יואב | פער שווה (2‑0 מול 1‑0 יחסית) → יואב בעל מכסה 1 מול רון שכבר לקח 1 |
| 3 | ב' (שניים) | רון | דנה ורון בפער שווה; `declaredAt` של רון מוקדם |
| 4 | ג' (שניים) | דנה | `want` > `can` |
| 5 | ד' (שלושה) | דנה | רון ויואב מיצו מכסה; דנה בפער 1 |

רון: 2 (מכסתו). דנה: 2. יואב: 1. **רון הצהיר ראשון על הכל וקיבל 2 מתוך 5** —
בדיוק החלק שלו — ובכל השאר הוא מספר 2 אוטומטית. זה הכשל שהתכנית נועדה למנוע.
...
- [ ] **P6 — הטופס והתחייבות המשמרות.** `ShiftPlanForm`, חישוב "כמה אנשים",
      ...
      וברכיב `negoarch` (1.0b), בטופס ההצטרפות, בקונטרה, בפינלייזרים
      (העתקה מהסבב האחרון) וב‑`proposeObjectEdit`.
- [ ] **P7 — כרטיסי lev וסולם החורים.** חמשת ה‑`ani`, `cardKinds.js`, שחרור,
      קידום גיבוי לאורך כל השרשרת, **כרטיס החור עם שתי האפשרויות** (§7.1,
      שתיקה = פתיחה מחדש), הרצת השידוך מחדש אחרי פתיחה.
