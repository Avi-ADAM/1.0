...
את **התנאים האלה** בלבד ומתאיינת ברגע שנפתח סבב חדש.

**היעד (⏳):** אין הצבעה שלילית "חשופה" על מועמדות. מי שלא מסכים פותח **סבב
מו"מ** (`counterOnAskm`/`counterOnAsk`) עם התנאים שכן היה חותם עליהם, והדרך
היחידה לדחות מועמד לגמרי היא **לקבל הצעה אחרת לאותו משאב** — קבלה של Askm מתחרה
כבר מארכבת את ה-Askmים האחיים (`runResourceAskmAcceptance`), כך שהדחייה היא
בחירה חיובית בין הצעות ולא וטו על אדם. כשזה ייסגר: כפתור "לא בעד" ב-`reqtom`
יפתח את פאנל המו"מ, ו-`voteOnAskm` יישאר רק לשורות legacy. אותו דבר בצד המשימות
(`reqtojoin`, שם ה-`else` של `decline()` עדיין ריק).
