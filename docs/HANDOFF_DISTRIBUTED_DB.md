...
- **ProjectState** קיבל את מפות קטגוריה א': `missions` (pendm/mesimabetahalich/
  finiapruval הם *states* של MissionView אחד), `halukas`, `decisions`,
...
- **חיבור לזרימה** (שלב 6): `specs/s2b.ts` (מקור אמת יחיד ל-predicates) +
  ...
  createSale). actions עם חתימה פר-call-site (addVote, createSale) *לא*
...
(א) רציפות `epoch === current+1`; (ב) ל-space‏ `project:X` — ה-actor חבר פעיל
...
### T10 — Compaction: snapshots חתומי-קוורום (חסר, קריטי ל-S2b)
PLAN §13.5 קובע שב-S2b זה "לא אופציונלי" — אבל לא היתה לזה משימה. אחרי
ש-T4 מכניס ~25 סוגי אירועים הלוג יגדל מהר. לבנות על מנגנון
`snapshot.commit`/`snapshot.vote` הקיים (אותו בסיס של T6): snapshot
תקופתי חתום ע"י קוורום הופך לנקודת-גיזום; לקוח חדש טוען snapshot + זנב,
לא את כל ההיסטוריה. **קבלה**: רפליקה חדשה מתכנסת ל-stateRoot זהה משני
המסלולים (היסטוריה מלאה vs snapshot+זנב).

### T11 — סגירת מרוץ ה-rotate (היום: "אמור לזהות" — לא ממומש)
§1 מתאר ששני `epoch.rotate` מקבילים מוכרעים דטרמיניסטית, אבל "המפסיד
אמור לזהות ולרוטט מחדש" לא ממומש ולא נבדק. אירועים שנחתמו במפתח המפסיד
בחלון המרוץ עלולים להיות בלתי-קריאים לחצי מהקבוצה. לממש: זיהוי הפסד
ברפליקה ⇒ re-wrap של המפתח הזוכה (או rotate חדש) + טסט שמריץ את המרוץ
בשני סדרי הגעה ומוודא שכל האירועים קריאים לכולם בסוף.
