...
ומשתמש קיים שרוצה להפעיל את הרקמה שלו בלי לפתוח דפדפן.
...
כלים עם תיאור של שורה, בלי מושג מה זו רקמה, שאסור להצביע בשם מישהו, ושתיקון
...
1. ✅ **נפתר — אוסף `api-keys` ב-Strapi.** המפתחות נוצרים והחיבור עובד (אומת 9.9.2026). מה שנכתב כאן למטה נשמר כתיעוד של החקירה, לא כחוסם פתוח.

   ~~**אוסף `api-keys` ב-Strapi ריק — אפס שורות.**~~ זו סיבת השורש לכך שהמפתח
...
2. ✅ **בוצע — `findUserProjectsTool`.** `userId` הפך לאופציונלי ונופל חזרה
   ...
   `userId` שרירותי ולקרוא את רשימת הרקמות של **כל** משתמש. עכשיו קורא חיצוני
...
3. ✅ **בוצע — הסרת `ask_*` / `run_*`.** `agents` ו-`workflows` כבר לא נחשפים
   ...
   מפורשים בלבד.
4. ✅ **בוצע — `createTaskTool`.** עדיין כותב עם `ADMINMONTHER`, אבל
   `createTask` מוגן ב-`projectMember` על בעל המפתח. **נחשף תמיד** (ראה 5):
   יצירת מטלה היא הדרך הרגילה של חבר לקדם משהו חדש ברקמה, ולכן תיאור הכלי
   מכוון את הסוכן להציע אותה תמיד ולבחור צורה — מטלה על משימה בתהליך (בעל
   המשימה + `missionId`), מטלה בלי מיישם עם רשימת תפקידים מתאימים (אחד
   מבעליהם אוסף אותה), או משימה דרך `prepareMissionTool` כשצריך למצוא מיישם.
5. ✅ **בוצע — סינון לפי scopes.** הכלים מסווגים לפי רדיוס הפגיעה:
   `read` ו-`prepare` תמיד; `selfWrite` (טיימרים — נוגע רק בבעל המפתח) תמיד;
   `consentWrite` (`createTaskTool`) תמיד — מטלה שמוקצית לאדם אחר ממתינה
   לאישורו (`myIshur`), ומטלה לתפקידים ממתינה שאחד המחזיקים יאסוף אותה, כך
   שאיש אינו מחויב בלי הסכמה; **`sharedWrite` (`createMissionTool`, שמפרסם
   ישירות בלי טופס לאישור) רק עם scope `mcp:write`**. מפתח מ-`npx 1lev1-mcp`
   נוצר בלי scopes ולכן מקבל את הסט הבטוח.
...
- `getProjectContextTool` (שלב 1 ב-`PLAN_AI_ERA`) נחשף גם ב-MCP — הוא מה שהופך
  "מה קורה ברקמה" מארבע קריאות לאחת.
...
### הריפו הנפרד מספיק — צריך רק להצביע אליו

`/plugin` היא פקודת טרמינל בלבד: היא לא קיימת ב-Claude Code שבדפדפן, ולכן סשן
ענן לא מגיע למרקטפלייס מעצמו. **זה לא אומר שצריך עותק של ה-Skill בריפו הזה.**
סשן ענן כן קורא `.claude/settings.json` מהריפו שהוא משכפל, ושם אפשר להצהיר על
המרקטפלייס ועל הפלאגין:

```json
{
  "extraKnownMarketplaces": {
    "1lev1": { "source": { "source": "github", "repo": "Avi-ADAM/1lev1-agent" } }
  },
  "enabledPlugins": { "1lev1@1lev1": true }
}
```

שש שורות בכל ריפו שעובדים בו, ו-`1lev1-agent` נשאר מקור האמת היחיד — בלי עותק
ובלי drift. (בסביבה מקומית המפתח צריך לסמוך על התיקייה כדי שהמפתח הזה יחול.)
מה שזה **לא** פותר: אפליקציית Claude ו-claude.ai הרגילה. שם Skills נטענים
בהגדרות החשבון, לא מריפו — וה-connector לבדו מביא כלים, לא שיקול דעת.
...
| README של רקמות ציבוריות | badge "מנוהל ב-1lev1" עם לינק להתקנה |
...
מפתח מתקין plugin (או שואל את הסוכן על 1lev1)
   ...
   → ה-Skill מציע: "להפוך את הריפו הזה לרקמה?"
   → createProjectTool מחזיר URL ממולא
   → הרשמה + יצירת רקמה בדפדפן
...
- רקמות שנוצרו מ-URL עם `?action=createproject` שמקורו בסוכן.
...
issues↔מטלות) לקהל שכבר יש לו רקמה.
