...
לחברי רקמה יש מערכות משלהם שמייצרות עבודה: מערכת טיקטים, לוח באגים, טופס
...
כמטלה ברקמה — משויכת למשימה בתהליך, מושמת לחבר או לתפקיד, וממתינה
**להסכמתו** של מי שאמור לבצע אותה.

מקרה־המבחן שממנו זה נולד: `106_ezrachi/src/lib/components/tikets/SupportWidget.svelte`
— ווידג'ט תמיכה שיוצר טיקט. הטיקט אמור להופיע גם כמטלה ברקמה של אותו אתר.
...
- `roleIds` — לאיזה **תפקיד** ברקמה (מי שמחזיק בו יראה אותה)
- `urgency` · `dateS` · `dateF`
...
|---|---|---|
| מפתחות API scoped לרקמה | `src/lib/server/apiKeys.ts` | `verifyApiKeyDetailed` עם `project`/`scopes`/`revoked`/`allowed_origins`, cache, `assertScope`, `touchLastUsed` |
...
מוצעת למחזיקי התפקיד ברקמה (`resolveRoleHoldersInProject`) — אף אחד לא
...
המערכת החיצונית (טיקט נפתח / באג דווח / טופס נשלח)
        ...
        │  Authorization: Bearer 1lev1_xxx     (מפתח scoped לרקמה, scope tasks:create)
        ▼
src/routes/api/v1/tasks/+server.ts                     ← חדש
        ...
        │  3. אידמפוטנטיות: externalId כבר קיים ברקמה? ⇒ 200
        │  4. המשימה/החבר/התפקידים שייכים לרקמה? ⇒ 404/403
...
- `missionId` חייב להיות `mesimabetahalich` **פעיל ברקמה של המפתח** ⇒ אחרת 404.
- `assignedUserId` חייב להיות חבר ברקמה ⇒ אחרת 403.
- `roleIds` חייבים להיות תפקידים המוכרים לרקמה ⇒ אחרת 404.
...
`awaitingConsent` (מושמת, טרם אושרה) · `open` (פתוחה לתפקיד/לרקמה) ·
`accepted` · `done`.
...
- `targets.ts` — "לאילו מפתחות ברקמה יש `callback_url`", עם cache בזיכרון
  (TTL 5 דק'). רקמה בלי יעדים חוזרת מיד ולא נוגעת ב‑Strapi. זה מה שמונע
...
זהו **עמוד ה‑API של הרקמה**, לא רק של המטלות.
...
שני מקורות עבודה ב‑106 ממופים לרקמה, דרך מודול משותף אחד.
...
- הלקוח שולח **רק `documentId`**; ה‑BFF קורא את ההצעה עצמה עם
  ה‑system token, כך שאי אפשר להכתיב מבחוץ מה יופיע ברקמה. השער האמיתי
...
## Phase 4 — SITE REPORT → הרקמה המרכזית (פנימי)
...
שחבר ברקמה לוקח אותה.

היעד הוא **הרקמה המרכזית, `projectId = 1`** (`SITE_REPORT_PROJECT_ID`
דורס אם צריך).
...
משתמש/ה מדווח/ת  →  POST /api/report
                     ...
                     │  3. mirrorSiteReport → createTask ברקמה 1   ← חדש
                     │  4. מייל אישור למדווח/ת, אם השאיר/ה כתובת    ← חדש
                     ▼
        חבר/ה ברקמה מאשר/ת את המטלה → updateTask
                     ▼
...
- מי מגיש את המטלה (`Act.vali`): `SITE_REPORT_TASK_CREATOR_ID`, ואם לא
  הוגדר — החבר הראשון ברקמה המרכזית (בקאש ל‑10 דק'). `createTask` חסום
...
- ניתוב אופציונלי: `SITE_REPORT_MISSION_ID` / `SITE_REPORT_ASSIGNEE_ID` /
  `SITE_REPORT_ROLE_ID`. בלעדיהם המטלה נשארת פתוחה לכל הרקמה.
...
קריאה בודדת של מצב המטלה. סדר הבדיקות נשמר מהזול ליקר: רקמה בלי
אינטגרציה **ואינה** הרקמה המרכזית עדיין עולה חיפוש אחד ב‑Set וקריאה
אחת מ‑Map מקושש, ולא נוגעת ב‑Strapi. רק רקמה 1 משלמת על טעינת ה‑Act.
...
כתובת, ואומר את הדבר האמיתי: לא "קיבלנו" אלא "זה נפתח כמטלה ברקמה
שמפתחת את האתר, ומישהו לוקח אותה על עצמו". לאורח שאין לו חשבון הכפתור
מוביל ל‑`/signup` עם הזמנה לקחת חלק; למשתמש רשום — ל‑`/lev`.

שליחת המייל והמיפוי שניהם fire‑and‑forget: הדיווח כבר נשמר, ולא רקמה
...
1. **`projectId` לעולם לא מהלקוח** — נגזר מהמפתח. הנזק המרבי ממפתח שדלף
   הוא הצפת רקמה אחת במטלות; אין קריאה, אין מחיקה, אין עדכון.
...
6. ולידציית שייכות (משימה/חבר/תפקיד לרקמה) בשרת בלבד.
...
- **ידני/curl**: מפתח תקין ⇒ 201 + התראה למשוּיָך; `externalId` כפול ⇒
  `duplicated:true`; משימה של רקמה אחרת ⇒ 404; מפתח בלי `tasks:create`
...
2. **מחיקה/סגירה מבחוץ.** טיקט שנסגר במערכת החיצונית — האם לסגור את
   המטלה? כרגע לא: סגירת מטלה היא מעשה של הרקמה. מועמד ל‑`PATCH`
   עתידי שפותח הצעת ארכוב (`PLAN_OBJECT_ARCHIVAL`) במקום למחוק.
3. **כמה מפתחות לרקמה** — הפעם ריבוי מפתחות בעלי שם (פר מערכת מחברת),
