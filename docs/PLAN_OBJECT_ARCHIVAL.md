# ארכוב ועריכה של אובייקטים ברקמה (Object Archival & Edit)

> **סטטוס: אושרה (סבב 1) — ביישום.** כל שמונה נקודות ההכרעה נענו; ראו
> "החלטות שהתקבלו" בסוף המסמך. השינוי המהותי מול הטיוטה הראשונה: זמן
> הרדימות הפך לפרמטר שנקבע ברמת הרקמה **וברמת המשימה** (סעיף 5), והוא
> פותח הצעת שחרור אוטומטית — ראו "רדימות".

## Context — למה זה נבנה

לרקמה יש היום תהליך יצירה מובנה ומוסכם לכל אובייקט:
...
להוריד מוצר מהמדף — אין לו דרך. ברקמה של אדם אחד זה לא נורא (הכול שלו,
הכול מיידי), אבל ברקמה של כמה אנשים **הסרת אובייקט היא שינוי בחוזה
...
> להיות צורך של הרקמה". בהתאם לעקרונות-העל:
>
> - **אין "לא" מוחלט.** לחבר רקמה יש שלוש דרכים להגיב להצעת ארכוב:
...
> - **שתיקה היא הסכמה, בקצב הרקמה.** הצעת הארכוב עומדת ל-`restime` של
...
> - **רקמת יחיד = מיידי.** כשקבוצת המסכימים פרט ליוזם ריקה, הפעולה מוחלת
...
| **הצורך של הרקמה** — שהאובייקט בכלל קיים | הרקמה | הסכמת כלל החברים |
...
לדיון, כי יש לו השלכה כספית על הרקמה.
...
kind        += 'archiveObject'   # הצעה להסיר
            ...
            += 'dormtoM'         # שינוי זמן הרדימות של הרקמה (אחות ל-timtoM)
...
archEndsMembership  boolean      # אישור ההצעה מסיים גם חברות ברקמה
archMember   relation oneToOne → users-permissions.user   # של מי (חד-צדדי)
...
`max(ordern)`. היא מבשילה כשכל חברי הרקמה חתומים על אותו `order`
ב-`vots`, או כשה-`timegrama` פוקע בלי "לא" מפורש. הצעת סבב = הסכמת המציע
לאותו סבב.

### 4. זמן רדימות — פרמטר של הרקמה ושל המשימה

```
Project.dormancyDays            integer   # ברירת המחדל של הרקמה; null → 30
...
- **ברמת הרקמה** — נקבע ביצירת הרקמה (לצד `restime`) ונערך אחר כך דרך
...
המיידי של רקמת יחיד. שכפול הלוגיקה בין השלושה הוא באג שמחכה לקרות.
...
### שינוי שווי שעה על משימה בתהליך — חיתוך עידני תעריף ✅ יושם

**מיושם** — `src/lib/timers/rate.ts` (החשבון הטהור),
`src/lib/server/timers/flushRateChange.ts` (החיתוך עצמו).

שעה שווה מה שהמשימה שווה **כשהיא נעבדה**, לא מה שהיא שווה כשמישהו סוף
סוף מאשר אותה. עד כאן התעריף נקרא מהמשימה ברגע הסגירה בארבעה מקומות, ולכן
`editObject` **אחד** שאושר תימחר מחדש כל שעה שהמשימה צברה אי־פעם — כולל
שעות שהרקמה חתמה עליהן חודשים קודם.

**חותמת שנוסעת עם השעות.** `Timer.rate` → `Finiapruval.perhour` →
`FinnishedMission.perhour`. בסגירה קוראים את הראשונה שקיימת
(`resolveRate`), ורק רשומה מלפני החותמת נופלת חזרה לשווי הנוכחי של
המשימה — כלומר **null = legacy = בדיוק ההתנהגות של היום שבו היא נכתבה**.

**שורה אחת לכל עידן תעריף.** שעות שנעבדו ב-₪50 מגדילות את שורת ה-₪50;
שעות ב-₪80 פותחות שורה חדשה (`pickRateRow`). ה-`total` מחושב מהתעריף של
**השורה**, לא מזה של המשימה. בסיום משימה השורות מקופלות לפי
`Σ (שעות × התעריף שלהן)` ולא `Σ שעות × התעריף האחרון`, וה-`perhour` של
השורה הסופית הוא הממוצע המשוקלל — היחיד ששומר על
`noofhours × perhour === total`.

**החיתוך עצמו, בתוך ההחלה.** כשסבב `keep` על `missionInProgress` מזיז את
`price`, לפני שנכתב השווי החדש:

1. הסגמנט הפתוח של הטיימר נסגר ברגע `T`, והטיימר מסומן `saved`.
2. המונה החודשי של המשימה סופג את חלקו של החודש, ו-`activeTimer` משתחרר.
3. השעות נשלחות לאישור ב**שווי הישן** — `finiapruval` עם `perhour` חתום
   ו-timegrama לפי ה-restime; ברקמת יחיד הן נזקפות ישירות לשורה של אותו
   עידן.
4. רק עכשיו נכתב ה-`perhour` החדש.
5. אם הטיימר רץ — נפתח טיימר חדש עם `start: T` (אותה חותמת זמן, אפס שניות
   אבודות) ו-`rate` = השווי החדש.

ה-`finiapruval` שנפתח **לא חוסם** את העריכה: מכיוון שהתעריף חתום עליו, הוא
יכול להמתין להצבעה בזמן שהשווי החדש כבר חי. החתימה הראשונה עליו היא של
מי שהשעות שלו — לא של מי שהחלטתו הבשילה.

**טיימר נשמר פעם אחת.** ה-store של הטיימרים בדפדפן מתרענן רק על הודעת
`timer*` בסוקט, וההודעה על הבשלת החלטה היא `voteUpdate` — כלומר הטאב של
בעל המשימה יכול להישאר עם מזהה הטיימר הישן אחרי החיתוך. לכן `timerSave`
קורא את הטיימר **לפני** שהוא נוגע בו, ואם הוא כבר `saved` הוא חוזר
`alreadySaved` בלי לפתוח `finiapruval` שני על אותן שעות. אותה קריאה מחזירה
גם את ה-`rate` החתום, כך שגם שמירה מטאב ישן מתומחרת בשווי הנכון.

מה שלא נחתך: שינוי שעות בלבד, שווי שנרשם מחדש ללא שינוי, ומשאב בתהליך —
המחזורים שלו (`maap`→`rikmash`) כבר צוברים כל אחד את הסכום שדווח עליו,
ולכן אין שם תמחור רטרואקטיבי מלכתחילה.

**נאמר לפני החתימה**, כמו `archEndsMembership`: `archive.edit.rateNote`
במגירת ההצעה ו-`archive.card.rateNote` בקלף ההצבעה.
...
### עריכה של אובייקט שכבר הגישו אליו מועמדות

**מיושם** — `src/lib/server/archive/pendingOffers.ts`.

משימה פתוחה / משאב פתוח יכולים לשאת מועמדויות תלויות: `ask`/`askm` שבעליו
אמר "אני לוקח" על **התנאים כפי שהיו כשהגיש**. הצעת `editObject` מוכרעת
בידי חברי הרקמה בלבד — המועמד אינו חבר ואינו מצביע עליה — ולכן אישורה
משנה בשקט את מה שהמועמד הסכים לו. בלי טיפול, השעון של המועמדות מבשיל על
*שתיקתו* ורושם עליו התחייבות שמעולם לא לקח: 40 שעות במקום ה-10 שהציע.

התיקון אינו מנגנון חדש אלא בדיוק זה שהרקמה כבר משתמשת בו — **סבב נגדי**.
כשהתנאים זזים מהותית, כל מועמדות תלויה מקבלת סבב צד-פרויקט
(`proposedBy: 'project'`) ב-`ordern + 1` הנושא את התנאים החדשים, חתום
בשם מי שעומד מאחורי הגרסה שהוחלה (proposing is agreeing, כמו
`counterOnAsk`). משם `computeNegoGate` עושה את השאר: הסבב העומד אינו סבב
שהמועמד חתם עליו, ולכן `takerYes` שקר ושום דבר לא מבשיל עד שיאשר בעצמו
או יציע נגד. השעון שלו מאופס כדי שיקבל restime מלא, והוא מקבל מייל שאומר
מה השתנה ומה התנאים כעת.

**מה נחשב "מהותי"**: רק שדה שבאמת *שונה* מהערך שאליו הוגשה המועמדות —
`name`, `descrip`, `hm`, `price`, `sqadualed`, `sqadualedf`. טופס העריכה
נפתח עם השעות והתמורה של היום, ולכן סבב ש*נושא* ערך לא בהכרח שינה משהו;
בלי ההשוואה הזו כל תיקון־טעות־כתיב בתיאור היה מבטל כל מועמדות ברקמה.
המצב נקרא **לפני** הכתיבה (`readOfferSnapshot`) — אחריה אין למה להשוות.

**גם מסלול האישור המפורש נסגר.** `evaluateAskAcceptance` חסם עד כה רק
הצעה מוקצית (`isRishon`); כעת הוא חוסם בכל מקרה שבו `takerYes` שקר, ולכן
חבר שלוחץ "אשר" אחרי עריכה מקבל `awaitingAssigneeConsent` — הצבעתו
נרשמת בסבב העומד וממתינה, במקום להתממש על תנאים שהמועמד לא ראה. אותו
שער נוסף ל-`finalizeAskmAcceptance`, שעד כה לא היה מגודר כלל בצד השרת.

### `matanot` — מתנה / מוצר

`lifecycle: 'archived'`. **מכירות קיימות ממשיכות להצביע עליו** — לעולם
לא מחיקה. מוצר אישי (`origin: 'personal'` + `owner_user` = אני) נשאר
ריבוני-מיידי דרך `archivePersonalMatanot` הקיים; רק מוצר של רקמה עובר
את ה-Decision. אם יש `monter` פעילים (מנויים) — הם נסגרים והלקוחות
מקבלים התראה, וזה נכנס לטקסט הקלף ("3 מנויים פעילים ייסגרו").

---

## כשהארכוב מסיים גם חברות ברקמה

אדם יכול להצטרף לרקמה בשביל דבר אחד בדיוק. אם זו המשימה (או המשאב)
היחידה שלו ברקמה **ולא נצברו בה שעות**, ביטולה מסיר את הקשר האחרון שלו
לרקמה — ולהשאיר אותו ברשימת החברים זה שקר: הוא ימשיך להיספר בכל קוורום
עתידי, ברקמה שאין לו בה שום נתח.

לכן: **ארכוב או שחרור של הקשר האחרון מסיים גם את החברות.**

**תנאי ה-0 שעות הוא מה שהופך את זה לבטוח.** חבר עם שעות רשומות,
`finnished-mission`, מכירה או חלוקה — צבר ערך ברקמה, וזה עצמו קשר. חבר
כזה לעולם לא יוסר במסלול הזה.

**מה נספר כקשר** (`assessMembership`, שאילתה אחת עם aliases):
משימה בתהליך אחרת · משאב בתהליך אחר · `finnished-mission` בפרויקט ·
`sale` שהוא מחזיק · `haluka` ששלח או קיבל · `ask`/`askm` פתוחים שממתינים
לו. אחד מאלה ⇒ החברות נשארת, רק האובייקט מתארכב.

**שקיפות לפני הכול.** ההשלכה נקבעת כבר בפתיחת ההצעה ונשמרת על ה-Decision
(`archEndsMembership` + `archMember`), כדי שהקלף וההתראה יאמרו את זה
במפורש לפני שמישהו חותם: *"זו המשימה היחידה של X ברקמה ולא נצברו בה
שעות — אישור ההצעה יסיים גם את חברותו"*. הסרה של אדם מרקמה לא תתגלה
בדיעבד.

**הסכמת הנוגע בדבר מובנית.** הקונסנזוס כאן פה-אחד, ולכן חתימתו של החבר
שחברותו על הפרק נדרשת ממילא — אין צורך בכלל מיוחד.

**נבדק מחדש בהבשלה.** ה-flag על ה-Decision הוא הצהרה לממשק; ההחלטה
בפועל נלקחת שוב ב-`apply.ts` ברגע ההחלה, כי במהלך ה-restime החבר עשוי
היה לקחת על עצמו משהו נוסף — ואז הוא נשאר.

**שני מעצורים:**
- **רקמה לא מתרוקנת** — החבר האחרון לעולם לא מוסר (`isOnlyMember`).
- **כישלון בבדיקה לא מבטל ארכוב** — אם בדיקת החברות נופלת, האובייקט
  מתארכב בכל זאת והחבר נשאר ברשימה: מצב שניתן לתקן, בשונה מאיבוד הארכוב.
...
- **המבצע עצמו** יכול תמיד לשחרר משימה רדומה ללא שעות **מיידית**
  ...
  הסגירה נפתחת כ-Decision רגיל להצבעת הרקמה.
...
- **חותמת התעריף** (ראו "חיתוך עידני תעריף"): `timer.rate` (decimal,
  `localized: false`) ו-`finiapruval.perhour` (decimal). שני שדות בלבד,
  ותוספת שדה לקולקציה קיימת לא דורשת שינוי הרשאות. עד שהם עולים כל
  הקריאות נופלות ל-fallback ומתנהגות כמו קודם.

**1.0:** `npm run types:update`, ואז `npm run check`.

**Exit:** הסכימה עולה, ה-codegen עובר, `STRAPI_SCHEMA_REFERENCE.md` מעודכן.

### פאזה 1 — פתיחת הצעה + רקמת יחיד + כיבוד `lifecycle` בסינון ✅ יושמה

**מה נכתב בפועל:**
`src/lib/archive/dormancy.ts` (טהור, משותף) ·
`src/lib/server/archive/{gql,targets,apply,decision,exec,membership}.ts` ·
...
5. Notification לכלל חברי הרקמה (`channels: socket, push`,
...
(`users_permissions_user`) ולכל חבר רקמה.
...
**Exit:** אפשר לפתוח הצעת ארכוב; ברקמת יחיד האובייקט נעלם מיד; ברקמה
...
היקף הקונסנזוס הוא **כלל-רקמתי** (בשונה מ-`saleClaim` הדו-צדדי). הצבעה
...
**שני תיקונים אחרי הרצה ראשונה (פאזה 2ב׳):**
1. **הקלף הראה רק את המוצע.** ההצבעה היא על ה*הפרש*, ולכן
   `buildTermRows` ב-`src/lib/archive/decisionView.ts` מחזיר לכל שדה
   את הערך הקיים לצד המוצע ואת הדלתא, והקלף מציג טבלת "התנאים" אחת:
   שדה שהסבב לא נגע בו מוצג כערכו הנוכחי בלבד, ושדה שזז מוצג
   `קיים ← מוצע (+N)`. הערכים הנוכחיים מנורמלים מחמש הקולקציות
   (`hoursassinged` / `noofhours` / `hm` / `quantityAssigned` /
   `quant`) — `currentOf` הוא בבואת הקריאה של `editFragment`
   ב-`server/archive/apply.ts`, ושתיהן משתנות יחד. אותו מקור מזין גם
   את רמזי "קודם:" בדראוור המו״מ, שהיו ריקים בהצעת ארכוב (סבב
   `archive` נושא תנאי סילוק, לא ערכי אובייקט).
2. **כפתור הצ'אט פתח כלום.** `Decision.forums` קיים בסכמה ו-
   `ensureVoteForum` כבר תומך ב-`entityType:'decision'` — הקלף פשוט
   העביר `{ decisionId }` ולא `{ forumId }`, ודף הלב נפל ל-fallback
   חסר ה-payload. עכשיו הקלף מייצר/מאתר את הפורום בלחיצה כמו
   `saleClaim` ו-pendm/pmash. אותו תיקון ב-`StipendDecisionCard`,
   ו-`VoteDetail` מזרים את ה-`forumId` ל-overlay הגלובלי במקום
   `noop`. **אין שינוי בסטראפי.**
...
- `dormancyDays` בטופסי יצירת הרקמה והמשימה + ענף `dormtoM` בהגדרות
  הרקמה לצד `restime`.
...
- **`editObject` מלא**: כפתור "בקש עדכון" ב-`missionInProgress.svelte`
  ...
  שווי שעתי, תאריכים. ברקמת יחיד מיידי, אחרת אותו קלף/מו״מ/timegrama.
...
3. **קונסנזוס כלל-רקמתי, פה-אחד או שתיקה** — אושר, "כמו בכל מקום יש
...
5. **זמן הרדימות הוא פרמטר, לא קבוע** — נקבע ברמת הרקמה (ביצירתה,
...
### Unit (Vitest)
- `proposeObjectArchive`: רקמת יחיד → `lifecycle: 'archived'` מיידית בלי
  Decision; רקמה מרובה → Decision + `negoarch[0]` + vote order 1 +
...
- `assessMembership`: קשר אחרון ⇒ `isLastTie`; כל אחד מששת סוגי הקשר
  מבטל אותו; שעות שנצברו מכריעות בלי שאילתה; החבר האחרון ברקמה לא מוסר.
- `endMembership`: כותב מחדש את `user_1s` בלי החבר ומשאיר את השאר; no-op
  כשהוא לא ברשימה; מסרב לרוקן רקמה.
- `applyObjectChange`: מסיים חברות כשזה הקשר האחרון (גם ב-`release`);
  לא מסיים כשיש קשר אחר או שעות; כישלון בבדיקה לא מונע את הארכוב.
- `dormancy.ts`: משימה עם שעות אינה רדומה; משימה עם טיימר פעיל אינה
  ...
  זה של הרקמה, ושל הרקמה על ברירת המחדל 30.
