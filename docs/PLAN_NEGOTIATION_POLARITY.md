...
|-------|---------|----------|-----------|----------|
| משאב פנימי | `pmash` | `nego-mash` (`isOriginal`, `users_permissions_user`) | חברי רקמה בלבד | אין — **חד-צדדי** |
| משימה פנימית | `pendm` | `negopendmission` | חברי רקמה בלבד | אין — **חד-צדדי** |
| מועמדות למשאב | `askm` | `nego-mash` (`ordern`, `proposedBy`) | מועמד ↔ רקמה | ✅ `proposedBy` |
| מועמדות למשימה | `ask` | `negopendmission` (`ordern`, `proposedBy`) | מועמד ↔ רקמה | ✅ `proposedBy` |
| שירות | `sheirutpend` | `sheirutnego` (`price`, `quant`, תאריכים) | לקוח ↔ ספק | נגזר מ-`users_permissions_user` |
| מתנות/רצון | `matanotpend`, `ratson-proposal` | `nego` (`proposedPrice`, `proposedQuantity`, `proposedHours`) | מציע ↔ רקמה | נגזר |
...
> אותו צד (הרקמה מול עצמה) — שם המודל מתנוון ל**התכנסות** במקום ל**הצטלבות**
> (§4.4).
...
- **`consumer`** — מי שמקבל אותו ומשלם (הרקמה, הלקוח).
...
`consumer`** (הרקמה מתמחרת מול עצמה) — ר' §4.4.
...
|------|-----|---------|-------|------|
| `pmash` / `nego-mash` | `price` | `provider` | ✅ | עלות לרקמה |
...
|------|-------|----------------|
| `high` / `low` | `max(p,c)` / `min(p,c)` + מי הציע | "הצד הגבוה: אתה, 300. הצד הנמוך: הרקמה, 200" |
...
**זה לב העניין.** דוגמת המשתמש: הרקמה מציעה 200 (`c=200`), המועמד מבקש 300
(`p=300`). `crossed=false`, `settled=200`, `gap=100`. ברגע שהרקמה תעלה
...
הרקמה **אינו מאפס** את הסכמת הרקמה (היום כל סבב `candidate` מאפס — ר'
...
כאן כל המציעים על אותו צד (`consumer`): הרקמה מגבשת מה היא מוכנה לשלם. אין
הצטלבות — יש **התכנסות**:
...
### 9.6 שאלות למשתמש
1. במסלול הפנימי (`pendm`/`pmash`) — הצגת ה"מינימום המוסכם" בין חברי רקמה
