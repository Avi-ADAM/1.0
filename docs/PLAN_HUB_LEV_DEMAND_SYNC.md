...
הצורך: משימה/משאב פתוחים יכולים להגיע משלושה מקורות — רקמה (project), משאלה
(concierge / ratson) או מאגד ביקוש (maagad) — וכל נקודות התצוגה היו מוטות-רקמה.
...
- `availableMission/[id]`: כותרת/זהות לפי מקור (רקמה / 🤝 מאגד / 🌟 קונסיירז'
...
- `availiableResorce/[id]`: היה קורס על משאב בלי project (גישה ישירה
  ...
  `source`/`ratson`. זרימת ה-Askm הישנה דורשת רקמה — למקור משאלה/מאגד
...
- `processSuggestions`/`processResourceSuggestions` ממתגים: `מאגד · <שם>` /
  ...
  במקום דיאלוג הרקמה) ו-`offerHref` (כשה-flow הרגיל לא זמין — הכפתור הראשי
