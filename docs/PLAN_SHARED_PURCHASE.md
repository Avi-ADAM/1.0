...
3. לאפשר לקבוצה לקבל החלטות *פר-פריט* ב-BOM שה-`extracted_*` / `ratson-proposal` ייצר.
...
| `consensusRule` | enum `['unanimous','majority','threshold','weighted','agreers_only']` (default `majority`) | כלל ההצבעה ברירת מחדל לכל החלטות בתוך ה-Ratson |
| `consensusThreshold` | Decimal 0..1 (default 0.66) | רלוונטי כש-`consensusRule='threshold'` |
...
**אילוצים**: כאשר `joinKind != 'solo'` ⇒ `allowJoin=true` חובה. `consensusRule='weighted'` דורש שאחד מ-`willingness` הוא מקור המשקל (ראה 3.4).
...
| שדה | סוג | תיאור |
| --- | --- | --- |
| `requires_consensus` | Boolean default true | האם הצעה זו דורשת הצבעה קבוצתית או רק אישור היוזם |
| `consensus_overrides` | JSON nullable | override של `consensusRule`/`threshold`/`fallback` ספציפי להצעה |
...
src/lib/server/consensus/evaluateConsensus.ts  (חדש)
  evaluateConsensus({
    ...
    votes: Vote[],
    rule: 'unanimous'|'majority'|'threshold'|'weighted'|'agreers_only',
    threshold?: number,
    weights?: Map<userId, number>
